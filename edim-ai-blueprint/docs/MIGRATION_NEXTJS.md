...
- **이관 48/59 화면**: +data-i18n(`/i18n/data/{type}?locale=` 원문/번역 인라인 편집 → PUT 서버액션, 대상·언어 전환)·detail/event(`/erp/events`+`/erp/events/{id}/flow` — 이벤트 목록·상세·공정 흐름 prev→cur→next). write 8. 남음: Run CostPanel·design-editor·ui-designer·plm-work-process·doc-template·print-setup·selection·detail(code/output)·mobile-preview·plm-quality. ※nginx 중지 중=빌드검증만.
