...
- **이관 45/59 화면**: +bom-compare(`/codes/bom-compare?base=&target=` 추가/삭제/변경 3분할 diff, Enter 비교)·detail/part(`/parts/detail`+`/drawings/{no}/bom` 병렬 — 속성·공정·치수바인딩·BOM 드릴다운). write 4. 남음: Run CostPanel·design-editor(Cvs)·code-relationship(Cvs)·ui-designer·plm-work-process·data-i18n·doc-template·print-setup·selection·detail(code/event/output)·mobile-preview·plm-quality. ※nginx 중지 중=빌드검증만.
