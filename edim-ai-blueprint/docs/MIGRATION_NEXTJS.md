...
- **이관 50/59 화면**: +plm/quality(`/drawings/{no}/verifications` 규칙 그리드 + 활성 토글 PUT + 측정값 자동판정 `/verify` POST)·plm/work-process(`/erp/work-process/materials`+저장 오버레이 병렬, MAKE/BUY 토글 → PUT 저장). write 11. 남음: Run CostPanel·design-editor·ui-designer·doc-template·print-setup·selection·detail(code/output)·mobile-preview. ※nginx 중지 중=빌드검증만.
