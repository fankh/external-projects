...
- **이관 43/59 화면**: +tech-data(`/tables/tech-data?airflow=&pressure=` 성능표, Enter 재조회)·eco-change(`/eco/changes` ECR/ECO 대장)·variant(`/codes/values?group=` 배리언트 상수, ?group= 전환)·projects(`/projects` 대장). write 4. 남음: Run CostPanel·design-editor(Cvs)·code-relationship(Cvs)·ui-designer·bom-compare·plm-work-process·data-i18n·doc-template·print-setup·selection·detail(code/part/event/output)·mobile-preview·plm-quality. ※nginx 중지 중=빌드검증만.
