...
| **https://edim.seekerslab.com/docs/** | **산출물 다운로드 포털** (29종, Basic Auth `edim`/`edim` — `/etc/nginx/.edim_htpasswd`) | `py docs/tools/make_docs_portal.py` → 커밋·푸시 → 서버 `git pull` 후 `sudo rsync -a --delete docs/ /var/www/edim/docs/files/ && sudo cp docs/portal.html /var/www/edim/docs/index.html` |
