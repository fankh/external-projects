...
| **https://edim.seekerslab.com/cpq · /plm · /code · /erp** | **EDIM 업무 앱 (edim-web — dense B안 실구현, mock API, 13화면+상세 4종 드릴다운)** | `edim-web/` 수정 → `npm run build`(dist 커밋) → 서버 `git pull` 후 `sudo rsync -a --delete edim-ai-blueprint/edim-web/dist/ /var/www/edim/edim-static/` |
