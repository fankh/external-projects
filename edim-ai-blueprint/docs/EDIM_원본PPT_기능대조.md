...
### U21. Head 메뉴 사용자 편집 (슬라이드 57-②) — ❌ 미구현
"Head Item 편집: 추가 편집 삭제 기능 (기업 사용자는 ERP 하부의 Item 만 가능)". 현재 헤더 드롭다운은 고정(navDropdowns), 좌측 패널만 사용자 정의(/prefs/leftnav).
- [ ] 헤더 드롭다운 항목 사용자 편집(추가/제거/재정렬) — leftnav 패턴 확장(/prefs/headnav), 권한 범위 내 리프만
- [ ] 편집 모달(현재 LeftNavEdit 재사용) + 기본값 복원

### U22. Hierarchy 저장 전 정합 점검 (슬라이드 57-⑧) — ❌ 미구현
"Tree 편집 작업 점검: DB 의 상호 관계 이상 유무 확인 후 저장". U18 이동 가드(자기/하위·시스템·교차 422)는 있으나 트리 전체 정합 리포트 없음.
- [ ] GET /hierarchy/validate — 주소 중복·고아 노드·참조 무결성(계층 주소 ↔ 코드 링크) 점검 리포트
- [ ] Hierarchy 패널 '점검' 버튼 + 이상 목록 표시(정상 시 저장/승인 진행 안내)

### U23. 문서 채번 규칙 (슬라이드 53 Document Code) — ❌ 미구현
"Document Code: EU-3-2020-450-6-21-4-SR-7" — 세그먼트(부문-유형-연도-프로젝트-차수 등) 조합 자동 채번. 현재 문서번호는 수동 입력.
- [ ] 채번 규칙 정의(테넌트 설정 JSONB) + POST /documents/next-no — 규칙 기반 다음 번호 발급
- [ ] 문서 등록 폼 '자동 채번' 버튼 (수동 입력 병행 유지)
