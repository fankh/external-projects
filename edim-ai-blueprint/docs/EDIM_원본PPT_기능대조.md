...
- [ ] BOM×수주 → 자재 소요량/시기 산출 (MRP 1차)
- [ ] 작업지시-일정-작업장 부하 스케줄링 뷰
- [ ] Capacity 분석 (작업장·인원 기준)

## U5. 창고·자재 관리 심화 (슬라이드 46) — 🔶 부분
...
- [ ] 자재 유통기한(lot 만료) 필드 + 만료 임박 경고
- [ ] 창고 정기점검 기록 (검사주기 필드는 있으나 실적 기록 없음)
- [ ] 대체 자재 연결 (part 대체 관계)
...
- [ ] QR 코드 생성/스캔 진입 (도면·문서·프로젝트 딥링크 — 웹 URL+QR 렌더로 1차 가능)
- [ ] Project 중심 대화(코멘트 스레드·이력) — 사내 SNS(슬라이드 57 "업무용 SNS"와 동일 요구)
- [ ] 설치·유지보수 검수 기록 (현장 체크리스트)
...
- [x] 판넬 **크기 조정** ✅ v21.4 — 트리 폭 드래그 리사이즈(140~420px, localStorage 영속, 라이브 E2E 220→300 유지). 아코디언은 후속
