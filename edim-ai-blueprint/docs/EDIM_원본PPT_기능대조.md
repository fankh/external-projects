...
### U34. 메뉴 폴더 추가 (사용자 지시) — ✅ 구현 (v33.1)
- [x] '#이름' 폴더 마커 ✅ — 목록 항목에 폴더 마커 삽입(문자열 배열 유지 → 백엔드·검증 무변경·하위 호환), 마커 존재 시 커스텀 폴더 트리로 SSR 렌더(마커 아래 리프 소속·빈 폴더 생략)
- [x] 편집 UI ✅ — 좌측 메뉴 편집 모달·테넌트 메뉴 관리 양쪽에 📁 폴더 추가 입력(↑↓✕ 로 배치·이동·삭제 동일). 라이브: 테넌트 CPQ [#영업지원, #문서작업] → 트리 5노드(폴더 2+리프 3) 정확 렌더·관리 화면 추가 동작, 설정 원복
- [x] 초안 폴더 자동 주입 ✅ v33.4~6 (사용자 리포트: pages should be in folder) — 편집 진입 시 마커 없는 목록/첫 마커 이전 루트 리프를 MENU_TREE 그룹(#Sales 등)으로 자동 재구성, 사용자 정의 폴더 구간은 보존. 라이브: 개인 목록(#test 포함 22) → 27행(그룹 5+#test+리프 21 전량)
- [x] 등록 버튼 폭 ✅ v33.5 (사용자 리포트) — RegisterModal 트리거 alignSelf:flex-start·fit-content(컬럼 flex stretch 전체 폭 수정, 전 화면 공통). 라이브: 캘린더 57px

### 레거시 React SPA 사용 중단 (사용자 지시, 2026-07-19)
- [x] /edim-static nginx 410 Gone 봉인(설정 백업 후 교체·reload) + autodeploy 레거시 rsync 단계 비활성. 파일은 /var/www/edim/edim-static 보관(서빙만 차단). 라이브: 410 확인·메인 앱 200 무영향
...
- [x] 폴더 보존·가림 경고 ✅ v31.3~5 — 커스텀 렌더 폴더 구조 유지(사용자 리포트 수정), 관리 화면에 현 계정 개인 설정 가림 ⚠ 경고+해제 버튼(모듈 단위, 타 키 보존). 라이브 E2E 5/5
- [x] SSR 렌더 전환 ✅ v32.4 (사용자 지시) — 4종 pref(좌측/헤더×개인/테넌트)를 레이아웃 서버 fetch 로 첫 HTML 부터 커스텀 트리 렌더(기본 트리 깜빡임 제거·클라 fetch 제거). 라이브: JS 미실행 HTML 에서 부분 목록 반영·해제 즉시 복귀 확인, 스모크 80/80
...
## 11차 조사 신규 태스크 (2026-07-19, 백엔드 라우트 317개 vs 프런트 참조 전수 대조 — U35)

### U35. 미배선 백엔드 기능 UI 복원 — ✅ 완료 (v34.13~19)
스캔으로 확인된 "백엔드는 완비·Next UI 미배선" 기능을 레거시 배선 여부 기준으로 복원. (계기: 라이브 스위트 트리아지 — 스위트가 참조하던 레거시 기능들)
- [x] 요구사항 접수(dev_requirement) 전체 이식 ✅ v34.13 — devMode 게이트 📝 모달(등록/목록·이미지 첨부 Ctrl+V·상태 변경), 라이브 21/21
- [x] 거래처 수정(PUT /companies) 다이얼로그 ✅ v34.13→34.17~18 — 더블클릭 수정. **실버그 발견**: dblclick 에 딸린 ?sel RSC 네비게이션이 이후 폼 액션을 무효화 → 클릭/더블클릭 판별(260ms 지연 네비)로 해소, 라이브 55/55
- [x] 고객 전달 패키지(/files/export-package, E2) ✅ v34.19 — Project Folder ⬇ 전달 패키지(산출물만 ZIP+매니페스트)
- [x] 블록 다이어그램 DXF(/cad/from-blocks.dxf) ✅ v34.19 — 제품 선정 블록 모드 ⬇ DXF
- [x] 프로젝트 중복검토(/erp/projects/check-duplicate, S-3-5) ✅ v34.19 — 등록 폼 중복검토 버튼·유사명 안내
- [x] ⌘K 통합 검색 F6 그룹 확장 ✅ v34.15 — 공급처·창고·Macro·사용자(+SETUP 게이트) 딥링크, 라이브 12/12
- [x] 감사 diff 하이라이트(F7/M-15-9) ✅ v34.18 — 변경 상세를 필드별 diff 테이블(변경 필드 하이라이트)로 승격
- 잔여 미배선(의도적): /health(인프라)·/cad/index-all(관리 배치)·/cad/arrangement.dxf(테스트 소스)·/erp/events/escalate-overdue(배치)·/drawings/{no}/variants(도면 상세 후속 검토)
