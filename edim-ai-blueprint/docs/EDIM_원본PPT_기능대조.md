...
### U24. 매크로 함수 마법사 UI (슬라이드 59·24 Toolbox Macro) — ❌ 미구현
백엔드 함수 카탈로그(GET /macros/functions — 명칭·시그니처·설명·자연어 키워드 검색, TBX-014)는 있으나 프런트 미배선. 원본: [함수 마법사] 다이얼로그(검색→설명→선택 삽입).
- [ ] Macro Studio(M-15) 식 편집기 ƒx 버튼 → 함수 마법사 다이얼로그(검색·설명·삽입)
- [ ] Sub Work Place Coding 패널에도 동일 진입점

### U25. 그래프 마법사 (슬라이드 59 [그래프 마법사]) — ❌ 미구현
