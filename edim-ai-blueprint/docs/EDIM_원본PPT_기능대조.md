...
> 회귀 게이트: 라이브 스모크 **70/70** (2026-07-19, edimsol.com — v25~v28 신규 기능 마커 11화면 추가 포함 전체 그린).
...
### U31. WORK INSTRUCTION 공통 문서 헤더 (슬라이드 9 양식 표) — ✅ 구현 (v30.0)
s09 의 BOM·견적서 목업 상단 공통 표(Category ISO 9001 | Issued by department | Language | Revision | Page | 문서번호 + Title 행)가 PDF 렌더에 없었음.
- [x] 공통 헬퍼 ✅ — _work_instruction_header(6열 메타+Title 행, 폭 자동 분할) 를 문서 관리 PDF(build_doc_pdf)·CLT 견적서(build_clt_quotation_pdf)에 적용, 기존 본문 자동 하향
- [ ] 기술자료·계산서 등 잔여 렌더 확산 — 양식 표준화 협의(Approval 툴바 건)와 함께
