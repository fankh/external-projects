...
현재: Work Process 화면 MAKE/BUY 토글·CAD Mapping(E5)까지. 공정 파라미터 입력·설계우선순위 테이블 없음.

- [ ] 공정 파라미터 CRUD (작업장·인원·스킬·시간)
- [ ] Item별 자재 흐름 정의 (warehouse·min stock·공급자·리드타임)
- [ ] 설계우선순위 테이블 (우선순위·기준점·오류체크 조건)
...
## U6. Print Set-up 완성 (슬라이드 50) — 🔶 부분

기획: 기본 양식 배치·Data 호출·**그래프 불러오기·Data 위치 설정**·워터마크(✅)·**Font/크기·용지 크기·색상(칼라/흑백)·머리글 바닥글 편집·여백**·Printer/PDF(✅)·**File 내보내기(Office)**.

현재: 양식 콤보+워터마크 토글+표준 머리글/바닥글 실렌더(PDF)만.

- [ ] 용지/여백/폰트/색상 설정 → 렌더 반영
- [ ] Office(xlsx/docx) 내보내기
- [ ] 양식 내 Data 위치(자리표시자) 편집기·그래프 배치
...
## U11. 셸 공통 소품 (슬라이드 57) — ❌ 미구현 (소형)

- [ ] Head에 **사용자 회사 로고 배치** (테넌트 로고 업로드)
- [ ] 사용자 변경: Frame/canvas **색상 테마** (언어는 ✅)
- [ ] 판넬 **아코디언/펼침·크기 조정** (트리 폭 고정 220 → 드래그 리사이즈)
