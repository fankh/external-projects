...
### U17. Work Process 설계우선순위 테이블 (슬라이드 44) — ❌ 미구현 (U3 잔여의 구체 UI 확인)
Inventory Management 절 실체 = **Dim.별 설계 파라미터 표**: 설계우선 순위·상위설계 우선자료·설계 기준점 설정(예: Inlet Cone – Bearing)·**설계 오류 체크**(예: ④ > 300). 행별 편집 아이콘.
- [ ] dwg_dimension 확장(우선순위·기준점·오류조건) 또는 별도 테이블 + Work Process 화면 3절 표
- [ ] 오류조건 위반 시 Design Editor/Run 경고 연동 (U2 순환 점검과 통합)

### U18. Hierarchy 트리 편집 심화 (슬라이드 64) — 🔶 부분
현재 등록/개명/삭제만. 원본: **우클릭 컨텍스트 메뉴**(보내기·잘라내기·복사·삭제·이름바꾸기·새로만들기·속성·정보), **노드 이동(잘라내기→붙여넣기, Address 추적 유지)**, 유형별 **심볼** 관리, Hierarchy 검색, History(작업자·일자·승인).
- [ ] 컨텍스트 메뉴 + 노드 이동/복사 (주소 재계산·하위 연쇄)
- [ ] 노드 속성/정보 다이얼로그 (심볼·비고·이력)
- [ ] 트리 내 검색

### U19. PCR 세부 비용 체계 + CLT 견적서 양식 (슬라이드 74) — 🔶 부분
...
- [ ] cst_pcr detail 확장(비용 트리 JSONB) + Report Center 다열 비교 표
- [ ] 견적서 렌더 CLT 양식 업그레이드 (표 기반 reportlab)

### U20. Relationship·BOM 소품 (슬라이드 36·70) — 🔶 부분
- [ ] Child Group 행별 **Data 링크 아이콘**(Tech/Variant/DWG — 코드 상세·CAD 뷰어 딥링크)
- [ ] 관계 화면 **도면 미리보기 패널**(선택 child 의 DXF 썸네일)
- [ ] **EBOM Run 버튼 분리**(BOM 만 재전개 — C-1 재전개와 구분 표기) + BOM Export
