...
Duct(~~층 선택·기술계산표~~ ✅ v23.1·~~수동 도구~~ ✅ U1/U2 편집 트랙(✎ 수동 조정))·Work Process(~~공정 파라미터~~ ✅ v21.4·~~CAD 매핑~~ ✅ E5·~~Coding~~ ✅ v34.34 실평가)·낙관적 잠금 안내(~~프로젝트 단계 409~~ ✅).
...
## P3 (사소 — 3순위)
~~화면 라벨 i18n 하드코딩(전 화면)~~ ✅ v34.9~19 (N7 — t() 키 전수 시드 1,890키·EN 크롬 잔존 0, check_i18n_en 33화면 PASS), F-key 라벨만 잔존(키 이름 자체는 만국 공통 — 실질 0), ~~로그인 부가 요소(로케일·테넌트·버전)~~ ✅ v34.37, ~~MDI 파라미터 다중 인스턴스~~ ✅ v34.36 (MDI_PARAMS 화이트리스트 — 선택 상태 파라미터 제외),
...
## 미배선 API 교차 대조 (2026-07-20, 백엔드 317 엔드포인트 대 프런트 호출처)
- **배선 완료 (v34.39~42)**: 견적/견적안 삭제·관계 DRAFT 회수·사용자 초대(1차) · 제품코드/거래처 batch(2차) · 이동원장(+trace 무인자 422 잠복 버그 수정)·마일스톤 요약·영업일 계산기·ECO 상세·Table 영향도·단가 resolve·PCR 실적 Δ(3차) · 도면 BOM 라인 편집(4차)
- **Superseded (의도적 미배선)**: `/history`(감사 조회 M-14-6A 와 동일 sys_history), `/cad/arrangement.dxf`·`/cad/from-blocks`(JSON — .dxf 프록시 형이 정본), `/drawings/{no}/blocks`(`/cad/view/{id}/blocks` 사용), `/arrangements/…/geometry`(전체 컴포넌트 PATCH 로 커버), `/cpq/runs/{id}/outputs`(runs/{id} 안에 포함), `/i18n/{locale}`·`/i18n/data/{type}/map`(정적 bundles.ts 가 정본), `/codes/{code}/supplier-codes`(부품 기준형 사용), `/menu/modules`·`/menu/config/{login}`(prefs/tenant 메뉴 체계로 대체)
- **운영/스케줄러용 (UI 불요)**: `/health`(배포 게이트), `/cad/view/{id}/index`·`/cad/index-all`(재색인 ops), `/erp/events/escalate-overdue`(배치 에스컬레이션)
