...
- [ ] **그리드 키보드 내비** — ↑↓ 행 이동·Enter 열기·Delete 삭제·Home/End·PgUp/Dn (role=grid·focusable 없음) **[H]**
- [ ] **스티키/고정 헤더** — 긴 목록에서 헤더 스크롤 이탈 **[H]**
...
### G4. 잔존 무반응·하드코딩 (현재 트리 실재 확인)
- [ ] `RunScreen.tsx:154-155` — 산출물 패널 `ZIP ⬇`·`폴더 열기` span 무핸들러 (Run 산출물 ZIP 스트림/Folder 이동으로 배선; /files/zip 재사용 가능) **[M]**
- [ ] `RunScreen.tsx:152` — 산출물 GroupBox 제목 `산출물 — PS-61313-5` 하드코딩(activeProject 반영) **[S]**
- [ ] `WorkProcessScreen.tsx:178` — "Table — Work Process" GroupBox `＋ ✎ ⬇` 장식 툴바 무핸들러 **[S]**
- [ ] `ProjectScreen.tsx` 샘플 행 하드코딩 · `EventDetailScreen` `'PS-612'` 폴백·데모 재배정 순환(의도적 데모 — 확인 후 처리) **[L]**
