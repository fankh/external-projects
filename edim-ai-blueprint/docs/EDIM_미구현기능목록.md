...
- [ ] 상태바 승인 대기 = `approvalService.inbox().length` 실카운트 (로그인 시 + 결정/요청 후 갱신) — **C11 첫 항목과 동일(미완)**
- [ ] LNAV To-Do '승인 확인' = 동일 inbox 카운트 배선 (신규 발견 — C11 미포함)
- [ ] LNAV To-Do 'PL 지연' = 부서 업무함 지연 이벤트 카운트 (dashboard 집계 재사용) — 데이터 소스(`erp/dashboard`) 기존재
- 데이터·API 는 이미 존재(inbox·dashboard 집계, Dashboard KPI 는 F10 에서 실카운트) — 셸 크롬만 미배선. 규모 S(하프데이).
