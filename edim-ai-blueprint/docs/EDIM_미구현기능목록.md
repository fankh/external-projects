...
**관찰 1건 (기능 결함 아님 — 데이터/문서 정합성, 사용자 판단 필요)**: 라이브 DB에서 데모 계정 `edim`이 **SETUP** 레벨(roles 없음).
`seed_v4`는 edim→ADMIN 승격을 의도하나 현재 ADMIN 은 `park.f` 1개뿐. `live_b21`(47행)이 edim 레벨을 `SETUP∨ADMIN` 로 허용하도록 작성돼 **의도된 데모 하드닝일 수 있음**.
단, [설치·배포 매뉴얼](EDIM_설치배포매뉴얼.md) §3.2·[관리자 가이드](EDIM_관리자가이드.md)는 edim=ADMIN 을 전제 — **정합 결정 필요**: (a) edim→ADMIN 복원(문서 유지) 또는 (b) 문서를 SETUP 데모+별도 ADMIN 로 정정. 라이브 DB 는 임의 변경하지 않음.
