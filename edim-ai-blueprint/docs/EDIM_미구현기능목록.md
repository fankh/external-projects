...
- [ ] 잔여: dwg_verification 규칙 자동 판정(측정값→Macro 평가→판정 제안) · QC 성적서 PDF(인증서 요구 PO 연계 ERP-017 certRequired) · Dashboard 이상 이벤트(erp_process_event) 정식 연계
...
- [ ] 잔여: Supersedure 자동 연계(신·구 도면 대체) · Table 영향도(B20) 영향 분석 합산 · 변경 이력 대장 전용 뷰
...
- [ ] 잔여: 프로젝트별 실적 귀속(project_id 연계) · 실적→PCR 반영 재계산 · C3 Dashboard 차이 위젯 · 차이 경보의 이상 이벤트(erp_process_event) 승격
...
- [ ] 잔여: 지연/임박의 이상 이벤트(erp_process_event) 자동 승격·C4 에스컬레이션 규칙 연계 · 마일스톤 자동 시딩(수주 전이 시 표준 5단계 생성)

### D8. 생산성 UX 팩 — 매일 쓰는 손맛 — 🔶 **핵심 완료 (v13.63, 승인함 일괄 처리)**
...
- [ ] 주요 그리드 XLSX export 전면 (단가·부품·도면·창고·업체·감사 — 현재 BOM·코드그룹·데이터테이블만) 〔UI 배선, API 검증 불가〕
...
- [ ] 잔여: optimistic lock 다른 쓰기(치수/Table/권한)로 확대 · 전용 감사 조회 화면(기간/사용자/작업 필터 + export, M-14-6 박스 분리 승격)
...
- [ ] 잔여: dwg_file/MinIO 객체 GC(현재 DB 행만 정리) · Folder 최신 Rev 기본 필터 · 테스트/업무 Run 구분 표기
