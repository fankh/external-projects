...
## A-2. 2차 백로그 (2026-07-10 전수 재감사 — B1~B15 완료 후 잔여)

> 3방향 감사: ① 프론트 no-op·mock 잔존 ② 백엔드 미사용 테이블 12종(grep 0회 검증)·OpenAPI 스펙 대비 미구현 op ③ 설계 문서(기능 179·요구 80·보완노트) 대조.
> 현황: DB 테이블 사용률 42/54 (78%) — 미사용 12개 · statusMsg 전용 버튼 약 13개 · 스펙 정의-미구현 op 다수.

### B16. 도면 상세·CAD 심화 (DWG-024~027 + dwg_approval)
- [ ] 도면 Viewer 탭 5종 완성 — 도면 대장 상세에 Variants·Referencers·Attachment 탭 (DWG-026/027; referencers API 는 B7 에서 구현됨 — 화면 탭만 배선)
- [ ] 설계 Simulation 우측 판넬 (DWG-024) — Design Editor 우측에 치수 변경→실시간 재계산 미리보기 (엔진 완료, UI 만)
- [ ] CAD 명령 확장 (DWG-025) — 특성 편집(색·선굵기·투명도)부터, Trim·Block 그룹화·단면 보기는 뷰어 범위 협의 후
- [ ] dwg_approval 개방 — 도면 단계별 승인 (WRITE→REVIEW→APPROVE step, 범용 /approvals 와 연계)
- [ ] dwg_document·dwg_part_relation 개방 — 도면 블록 콘텐츠(JSONB)·부품 정렬/접촉 조건 (Macro 규칙 연계)

### B17. 부품(Part) 마스터 + 도면 BOM
- [ ] prt_part CRUD + 부품 대장 화면 (part_no·spec·재질 FK·공급처 FK·표준품 여부)
- [ ] dwg_bom 개방 — 도면-부품 BOM (item_no·수량·조립 순서 assembly_seq — 부품 상세의 조립순서 ◆ 실데이터화)
- [ ] prt_supplier_code_map — 공급자 코드 매핑 (ERP-018: 발주 문서에 공급자 코드 표시)
- [ ] product_code_item 개방 — 제품코드 필수 슬롯 정의 (C-1 슬롯 구성의 DB 원천화)

### B18. 원가·수익성 (CPQ Cost 완결)
- [ ] cst_calc 개방 — Run 원가 상세 (재료비/가공비/직접비 detail JSONB → Run 산출물 원가 탭)
- [ ] cst_pcr — 수익성 기여도(PCR) 보고서 (사업유형별 sections·contribution_margin·EBIT, UNIQUE(selection_id, business_type))
- [ ] 견적 lifecycle — cst_quotation 생성/렌더 API (현재 조회만; POST + render.pdf)
- [ ] 재고 단가 4종 표기 확인·완성 (최고·최저·평균·최근 — M-12-5 표기 누락분)

### B19. ERP 창고·구매 상세 (ERP-017~021)
- [ ] erp_warehouse 개방 — 창고/저장위치 계층 화면 (REGION→PLANT→WAREHOUSE→STORAGE→SECTOR, 위험물 허용·검사주기)
- [ ] 구매 상세 필드 — 납품조건·운송수단·최소구매수량 (M-8-2 발주 다이얼로그 확장)
- [ ] "견적 요청 (QCR)" 실배선 — 현재 statusMsg 만 (공급처 알림 + 회신 대기 상태)

### B20. Toolbox 심화 — Macro CODING 모드
- [ ] tbx_macro 미사용 컬럼 개방 — flowchart_def·code_text·test_input/test_result·description_text (4-Way Sync 실저장: 지금은 수식만 영속)
- [ ] apply_type='CODING' 모드 — 코드 기반 매크로 등록·실행 경로
- [ ] tbx_macro_ref — 매크로 역참조 추적 (Macro 가 참조하는 Table/Code 목록 → 영향도 분석)
- [ ] 함수 자연어 검색 (TBX-014) — Macro Studio 함수 팔레트 로컬 검색 우선 (AI 확장은 키 입력 후)

### B21. 시스템·UX 마감 (no-op 버튼 실배선 + read-only 개방)
- [ ] sys_user_role 개방 — 사용자 다중 역할 할당 (M-14-6 확장, sys_role_permission 과 합성)
- [ ] Hierarchy 노드 편집 — POST/PATCH/move/DELETE (M-3-1 현재 조회만)
- [ ] 문서 채번 자동화 — doc_control allocate-code + 상태 전이 API (스펙 정의됨)
- [ ] no-op 버튼 실배선: M-14-6 초대 메일·비활성화(sys_user status) · 문서 상세 미리보기/Print(워터마크 렌더 재사용) · S-3-5 중복검토(실 중복 질의)·Print Set-up 호출(S-3-4 탭 열기) · S-1-4 Child 추가(code_relationship insert) · M-15-3 처리 Form 열기(tbx_ui_form 연동) · UI Designer 미리보기(폼 렌더)
- [ ] GET /auth/me·/auth/permissions — 스펙 정의 op (프론트 세션 복원·권한 조회 단순화)

### B22. 산출물 문서 + 스펙 동기화
- [ ] OpenAPI 스펙 ↔ 라우터 동기화 — 스펙 정의-미구현 op 목록 정리(구현 or 스펙에서 후속표기), 라우터 신규 엔드포인트(승인·도면·파일 삭제 등) 스펙 반영 후 재생성
- [ ] 배치/보고서양식 정의서 — 야간 배치(백업·정산) 명세 + 정기 보고서 렌더 양식 (진행현황 §4.2)
- [ ] 사용자 가이드 (모듈별: CPQ·PLM·Code·ERP·Toolbox·공통) + 관리자 가이드 (권한·잠금해제·백업·모니터링)
