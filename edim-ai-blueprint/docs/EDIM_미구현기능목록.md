...
- [ ] 프로젝트 콤보 4종 = 활성 프로젝트 표시(읽기) 또는 `projectService.list()` 실옵션+선택 시 컨텍스트 전환
- [ ] C-1 캔버스 '전체 47' = 실 BOM 전개 총수 · AHU_BLOCKS = arrangement_component 실블록 — **C11 마지막 항목과 동일(미완)**
- 성격: 장식용/오해 유발(항상 'Micron #7'·'전체 47' 고정). 규모 S~M. 데이터·API 는 F1·B13 에서 기존재 — 화면 배선만.
