...
- [x] `GET /documents/{no}/render.pdf` — build_doc_pdf, **S-1/S-2 CONFIDENTIAL 워터마크 강제** (pypdf 텍스트 추출로 게이트 검증; doc_no 중복 시 최신 행 렌더 버그 수정) · **워터마크 강화(v13.88)**: 단일 중앙 → 페이지 전면 대각 타일 반복(크롭·부분 캡처 방지, build_doc_pdf·build_lines_pdf 공용 `_draw_watermark`)
...
- [x] selection finalize + x_code_status 심사 흐름(x-code review UI) — 저장 시 X-code 판별→PENDING + 검토·승인 UI 완료 (하단 G3 **X-code 검토 UI (v15.0)** 참조)
...
- [x] 잔여 해소: 월별 매출/기여마진 추이 ✅ (v16.0 — `/erp/analytics` `monthlyOrders`: ORDERED 견적 계약액+PCR 기여마진 월별 12개월, Next Dashboard 패널·수주 없음 정직 표시, 스모크 마커) · PCR 보고서 PDF(RPT-07) ✅ (v15.3 Report Center `GET /reports/pcr/{id}.pdf` 로 기구현)
...
- [x] 잔여 해소 ✅ (v16.1): `backend-db` 잡(services: postgres:16) — `tests/test_db_api.py` **8케이스**: 승인 전이(승인→자산 APPROVED·반려 코멘트 필수·재결정 404·대상별 PENDING 1건 409·일괄 skip 집계) + 창고 계층(재귀 CTE depth/path 정합·REGION 최상위 강제·동급 배치 422·중복 409·유형 검증). TestClient+lifespan(fresh DB 자동 migrate+seed), DATABASE_URL 없으면 skip(backend-unit 무영향), 재실행 멱등(로컬 2회 green). 커버리지 리포트(--cov=app term-missing+xml 아티팩트, 전체 33%). 부수: CI 레거시 경로 수정(edim-web→edim-web-react — 리네임 후 적색이던 CI/Nightly 복구) + `web-next-build` 메인 콘솔 빌드 게이트 신설
...
### C13. 문서 파이프라인 개선 (감사 권고) — 🏁 **완료 (v16.4)**
...
- [x] RTM C-단계 확장 → 규칙 명문화로 갈음 ✅ (v16.4) — RTM v0.2 문서정보에 "추적 범위" 명문화: RTM=요구사항(REQ-F/NFR) 중심, 구현 증분(B~G 배치)=본 문서가 배치 단위 추적, RTM 형식 확장 없음
...
- [x] 수주 시 프로젝트 영업 단계 자동 전이 + 후속 이벤트 생성 (v17.2) — 영업단계 CONTRACT 전이는 D1 완료분, 잔여였던 **후속 착수 이벤트 자동 생성** 구현: 견적 ORDERED 시 `_seed_order_followups` 가 OR(수주)의 erp_process_edge 후행(AP 승인도서·PL Part List)을 프로젝트 TODO 이벤트로 시딩(설계 착수 TODO), 프로젝트별 중복 방지(idempotent)·담당자(PM 우선) 알림·리드타임 due, response `followupEvents` 반환 + 수주 관리 화면 전환 메시지에 건수·명 표기. **E2E: live_g9_order_followup 7/7(승인 도서 신규 생성·Part List dedup·TODO 시딩·재수주 [] 중복 방지·업무함 반영·psql 정리)**
...
- [x] 재고 단가(STOCK) 자동 적재 연계 (v17.8) — **alembic 0022**(inv_stock.unit_price) + 입고 단가 지정 또는 cst_price(STOCK, 유효기간·item_code→part_no/product main_code 매칭) 자동 적재 → 이동평균 단가 갱신·평가액(수량×단가)·음수 422 + /erp/stock unitPrice/value·inbound 응답 avgPrice/priceAuto/value + 재고 화면 입고 단가 입력·단가/평가액 컬럼·총 평가액. **E2E: live_g14_stock_valuation 11/11(이동평균 100→150·자동 1500·평가액·422·목록 반영·psql 정리)**
- [x] Mobile '입고처리' 실배선 (v18.4) — Mobile App 미리보기(M-16) 자재 입고: QR 스캔(클릭=품목코드 입력)·수량 편집 → '입고 처리'가 실 재고 입고(inventoryService.inbound: inv_movement IN + inv_stock·단가 자동·평가액) + 연계 이벤트 완료. **E2E: live_g20_mobile_inbound 5/5(QR 영역·클릭→실 재고 +2·MI-MOBILE IN 이력·재입고 누적)**
...
- [x] Table 영향도(B20) 합산 (v13.90) — ECO DRAWING 영향분석에 **치수 매크로가 참조하는 데이터 Table 수(tableRefs)·치수 매크로 수** 병기(tbx_macro_ref TABLE 집계). **E2E: KDCR 3-13 tableRefs 1(DIM D=Table12) 라이브 검증**
- [x] 변경 이력 대장 전용 뷰 (v17.6) — GET /eco/ledger(전체 ECO 라이프사이클 대장 + 상태 집계, status/targetType 필터, 변경유형 파생: APPLIED+revTo=Rev-up·APPLIED+revNull=대체·REJECTED=반려·else 진행, Rev 전이 표기) + 변경 이력 대장 화면(D-5L, PLM>Design management) KPI(총/적용/진행/반려)·상태·대상 필터·헤더 컬럼 필터·CSV. **E2E: live_g12_eco_ledger 14/14(summary 구조·ECR→총계+1·진행 파생·Rev 전이 '—'·status/targetType 필터·psql 정리)**
...
- [x] 프로젝트별 실적 귀속 (v17.7) — **alembic 0021**(cst_actual.project_no) + POST /cost/actuals projectNo(존재 검증 404)·GET /cost/actuals?project= 필터·GET /cost/variance?project=(실적 귀속·추정 프로젝트 Run 스코프, `_latest_cost_base` project 인자로 run→selection→project 조인) + 원가 실적 화면 프로젝트 필터·적재 귀속·프로젝트 컬럼·차이분석 스코프 표기. **E2E: live_g13_actual_project 13/13(귀속 2건 필터·미귀속 제외·404·422·프로젝트 실적 합계 2500·분류별)**
- [x] 실적→PCR 반영 재계산 (v18.2) — GET /cost/pcr/{id}/actual: PCR 프로젝트 귀속 실적(cst_actual) 합계로 직접비 치환(매출 고정)→기여마진·EBIT 재산출 + 추정 대비 차이(마진율 델타), 실적 없으면 actualAvailable=false·404 + 원가실적 화면 '실적 반영 PCR' 패널(추정 vs 실적 매출·직접비·기여마진·EBIT·마진율). **E2E: live_g18_pcr_actual 13/13(구조·항등식·실적 적재 반영·직접비/마진/EBIT/차이 재계산·404)**
- [x] C3 Dashboard 차이 위젯 (v18.3) — /erp/analytics 에 variance 블록(분류별 추정=Run cst_calc·실적=cst_actual·차이·차이율·임계 +10% 경보·hasActual) + 대시보드 견적 vs 실적 차이 위젯(총 추정/실적/차이 + 분류별 추정→실적 차이율 바). **E2E: live_g19_dashboard_variance 10/10(블록·분류 3종·총계 항등·실적 적재 반영 재계산)**
...
- [x] 그리드 다중 선택 일괄 작업 패턴 확장 (v17.1) — 공용 `ConfirmDialog`(파괴적/일괄 확인, danger 스타일) + POST /codes/products/batch(STATUS 상태변경 | DELETE, 참조 있으면 skip 사유 반환, 최대 200)·POST /companies/batch(is_active 일괄 토글, 최대 200) + 제품코드 마스터 다중선택·일괄 승인/비활성/삭제(참조 skip 리포트)·거래처 대장 다중선택·일괄 비활성/재활성 (DenseGrid multiSelect 체크박스·Shift 범위·전체선택 활용). **E2E: live_g8_bulk_ops 15/15(승인 3건·삭제 3건·빈대상/action/status 422·거래처 비활성→active_only 제외→재활성·psql 정리)**
...
- [x] 잔여 판정·해소 (v34.52, 2026-07-20): **Table 행 편집 낙관적 잠금** — 스키마 무변경 baseValues 스냅샷 대조(PUT /tables/{name}/rows/{key}, 선수정 409 + 화면 재조회 안내). **단가 = 불요 판정**(insert+supersede 이력 적재 패턴 — 수정 API 없음, 충돌 개념 부재). **치수(PUT /drawings/dimensions) = 후속 판정**(협업 편집 요구 확정 시 — 현 단일 사용자 데모에서 토큰 도입 실익 없음)
...
- [x] ~~잔여: 모듈 하위 메뉴(노드) 단위 세분 표시 제어~~ → **superseded (v21~v29 네비 개편)**: 좌측/헤더 사용자 메뉴 편집(U21)·테넌트 기본 메뉴(U30, M-14-6B)가 노드 단위 표시 제어를 제공. Head-Templet 연결(처리 Form head_key 매핑)은 P5 협의 대상으로 이관
...
- [x] 잔여 해소 (v34.51, 2026-07-20): **파일별 CONFIDENTIAL 워터마크 실적용** — 고객 전달 패키지의 PDF 산출물에 전면 대각 워터마크 오버레이(pypdf 병합 `_stamp_pdf_confidential`, 손상 PDF 는 원본 유지 부분 실패 허용, X-Watermarked 헤더·매니페스트 고지). ~~doc_control Grade 연계 필터~~ → **문서함(M-5-4) 관할로 판정**: dwg_file↔doc_control 실링크가 스키마에 없고, Grade 통제(열람 403·워터마크 미리보기·그리드 필터)는 문서함이 담당 — Folder 는 파일 채널로 패키지 워터마크가 커버
...
- [x] **C-1 캔버스 arrangement_component 실블록화 (v13.87)** — **alembic 0013**(arrangement_component pos_x/pos_y/width/height, 데모 sort_order 자동 2열 배치, C6 11번째 증분 마이그레이션 0012→0013 라이브 자동적용). `GET /arrangements/{code}/components` 좌표 노출(미지정 자동배치 폴백) + `PATCH .../{id}/geometry`(드래그 좌표 저장). C-1 구성도가 서버 컴포넌트(arrBlocks)를 실블록으로 렌더(AHU_BLOCKS 폴백). **E2E: ARR-DD2 3블록 좌표 반환·PATCH (200,120,140×60) 저장·미존재 404 라이브 검증**
...
## A-9. 9차 백로그 — 병렬 심층 감사 (2026-07-12, CAD·UX·도메인 4-트랙 에이전트 프로브)

> 방법: 4개 탐색 에이전트 병렬(① 표준 CAD 기능 ② 단축키·그리드 UX ③ 도메인/CRUD 완결성 ④ 무반응·placeholder). 이미 완료된 D1~D10·E1~E6·F1~F10·심화(XLSX·optimistic lock·prefs·이상통합 등)는 **중복 제거**. 아래는 **현재 트리 기준 실제 미구현** 항목만.

### G1. 캔버스 편집(CAD) — 뷰어는 충실하나 편집 부재 〔CadSvg 읽기 전용〕
- [x] **블록 이동 (v13.94)** — Cvs 블록 드래그 이동(스케일 보정) → C-1 구성도 `geometry PATCH` 영속(arrangement_component). 서버 블록(componentId) 대상. CadSvg(DXF)는 별도 후속
- [x] **엔티티 편집 1차 — 이동·삭제 (v14.0)** — `POST /cad/view/{id}/edit`(ezdxf 재읽기 · entityId=인식엔티티 순번 매핑 · `translate`/`delete_entity` → MinIO 재저장 · 재파싱 반환, DXF 한정 501·SETUP·감사 CAD_EDIT) + CadSvg ✎ 편집 모드(선택 드래그 이동 라이브 프리뷰·Delete/🗑 삭제) + CAD 뷰어 배선(canWrite·DXF). **E2E: live_g1_cad_edit 10/10(이동 좌표 델타·삭제 −1·재-GET 영속·없는 id 422·정리)**
- [x] **엔티티 편집 2차 — 복사·회전·미러 (v14.1)** — `/cad/view/{id}/edit` op 확장(copy=복제+translate·rotate=중심 z_rotate(°)·mirror=중심 축 반사, ezdxf bbox 중심 pivot) + CadSvg 편집 패널 ⧉복사·⟳회전90°·⇋좌우반전. **E2E: live_g1_cad_edit2 12/12(복사 +1·회전 길이보존·미러 반사 항등식·없는 id 422·정리)**
- [x] **엔티티 편집 3차 — 트림/연장 (v14.6)** — `/cad/view/{id}/edit` trim op(대상 선의 가까운 끝점을 경계선 무한연장 교점으로 이동=트림 또는 연장, LINE 한정·평행 시 무효) + CadSvg ✂ 트림 툴(경계선 클릭→대상 끝 클릭·경계 파란 하이라이트). **E2E: live_g1_cad_trim 9/9(단축·연장·평행 422·영속·UI)**
- [x] **다중/마퀴 선택 (v14.2)** — 편집 모드 빈곳 좌드래그=박스 선택(bbox 교차)·Shift+클릭 토글·다중 하이라이트·중버튼 팬 → 선택 전체 일괄 이동/삭제/복사/회전/미러. **E2E: live_g1_cad_marquee 9/9(마퀴 다중 선택·🗑 일괄 삭제·영속·다중 op 배열)**
- [x] **자유 작도 (v14.3)** — `/cad/view/{id}/edit` add op(line/circle/rect→ezdxf add_line·add_circle·add_lwpolyline(closed)·레이어 자동생성) + CadSvg 작도 툴 ／선·○원·▭사각(드래그 러버밴드→DXF 추가·crosshair·Esc). **E2E: live_g1_cad_draw 11/11(좌표·반경·닫힌 4정점·영속·UI 드래그)**. polyline 다정점은 후속
- [x] **인쇄/플롯 to scale (v14.4)** — `GET /cad/view/{id}/plot.pdf?scale=&paper=&orient=`(1:scale 벡터 PDF·reportlab, line/circle/arc/polyline/text·A4/A3·가로세로·테두리·타이틀블록) + CAD 뷰어 축척/용지 선택·🖶 축척 PDF. **E2E: live_g1_cad_plot 9/9(PDF 시그니처·축척/용지별·기본축척·404·UI 버튼)**
- [x] **그리드 오버레이+토글 (v13.99)** — CadSvg ▦ 토글 → nice-step(1/2/5×10ⁿ) 격자·원점 축 강조(뷰 폭 기준 자동 스텝). **E2E: live_g1_cad_view 격자선·재클릭 해제·좌표 HUD grid step 병기**
- [x] **드로잉 단위 좌표 실시간 표시 (v13.99)** — 커서 이동 시 X/Y 도면 단위 HUD(CadSvg toDrawing 노출·이탈 시 소거). **E2E: 표시·값 갱신·이탈**
- [x] **객체 스냅 + Ortho (v14.5)** — 스냅 후보 끝점·정점·중점·중심(snapPoints midpoints) → 작도 시작/끝점 자동 스냅·노란 스냅 마커 · Shift=Ortho(수평/수직 제약). 측정도 중점 스냅 반영. **E2E: live_g1_cad_snap 6/6(끝점·중점 정확 스냅·Ortho 축정렬)**
- [x] **교차점 스냅 + Polar (v16.4)** — snapHit 선-선 교차점 후보(인접 엔티티 한정, 측정/작도 반영) + 작도 Shift=Polar 45° 8방향 각도 스냅. **E2E: live_g1_cad_snap2 5/5(교점 정확 스냅·45° 정렬)**. perp(수선) 스냅은 후속
- [x] perp(수선) 스냅 (v18.0) — 작도 시 시작점에서 커서 근처 선/폴리라인 세그먼트로 내린 수선의 발에 스냅(`perpFoot`·`perpHit` 커서 근처 세그먼트 한정, `drawEnd` 점스냅→perp→Polar 우선순위), ⊥ 글리프 인디케이터(파란 마커·`data-cad-perp`)·미리보기/커밋 공통. **E2E: live_g16_cad_perp 7/7(최소 단일선 DXF·⊥ 인디케이터·끝점=수선의 발·그려진 선 ⟂ 대상 |cos|=0)** **[L]**
- [x] **인터랙티브 치수 기입·블록 삽입 (v15.5)** — /cad/view/{id}/edit add op에 dim(치수선+거리텍스트@중점, DIM 레이어)·block(라벨 박스=사각+텍스트, BLOCK 레이어) + CadSvg 작도툴 dim/block(드래그 러버밴드) + Design Editor 치수 DI·Block REG 배선. **E2E: live_g1_dim_block 11/11**. 블록 팔레트(사전 정의 심볼)는 후속
- [x] **Design Editor CAD 명령 툴바 실동작 (v15.4)** — 무반응이던 이동/복사/반전/연장/삭제/회전/자르기 버튼을 CadSvg 편집 엔진에 배선(POST /cad/part-drawing/save 로 부품도 dwg_file 실체화→editFileId·CadSvg activeTool 제어 prop·onEdit 영속). Block REG·치수 DI 는 후속 안내. **E2E: live_g1_design_edit 10/10**
- [x] DuctDesign 실엔진화 + 인터랙티브 치수/Block (v18.1) — 정적 div mock → 서버 작도 DXF(`build_duct_layout_dxf`: ROOM·NOZONE·DUCT·DIFFUSER·AHU·DIM 레이어, ezdxf)·GET /cad/duct-layout(diffusers/floor, 12 상한·1 하한 클램프) → **CadSvg 실엔진 렌더**(줌/팬/레이어)·자동배치/Diffuser 재생성·편집 모드 치수 DI·Block 인터랙티브(onEdit `applyCadOps` 로컬 적용). **E2E: live_g17_duct_layout 11/11(레이어·DIFFUSER 원 수=diffusers·ROOM/DUCT 폴리라인·bounds·클램프)** **[M]**
- [x] Cvs·CadSvg 엔진 통합 (v18.5) — 블록 모델(CanvasBlock[])을 정규화 DrawingDocument 로 수렴(ARCHITECTURE §1 원칙 충족): 클라이언트 브리지 `blocksToCadDoc`(cadBridge.ts, div y-down→도면 y-up 반전) → CadSvg 실엔진 단일 렌더 + 서버 `build_blocks_dxf`(ezdxf, 닫힌 폴리라인·라벨)·POST /cad/from-blocks(정규화 문서)·/cad/from-blocks.dxf(익스포트, 빈 blocks 422) + 재사용 컴포넌트 `<BlockCadView>`(CadSvg + DXF ⬇) · Code Relationship 구성도 블록/CAD 실엔진 토글 · ARCHITECTURE 명문화. **E2E: live_g21_engine_unify 13/13(블록 2→닫힌 폴리라인·라벨/치수/자유 텍스트·y반전(-12/-72)·레이어·422·유효 DXF)**
- [ ] 네이티브 DWG read/write(ODA 플러그블 501 대기) · 축척비(1:100) 표시 **[L]**

### G2. 그리드·단축키 UX 심화 〔DenseGrid 공용 강화〕
- [x] **그리드 키보드 내비 (v13.92)** — ↑↓ 행 이동·Home/End·PgUp/Dn·Enter=열기(tabIndex·선택 행 scrollIntoView) — DenseGrid 공용
- [x] **스티키 헤더 (v13.92)** — `.g th` position:sticky(전 그리드 공용)·그리드 포커스 링
- [x] **전역 단축키 (v13.93)** — Ctrl+S(저장 F12)·Ctrl+F(검색)·Ctrl+P(인쇄)·Delete(삭제 F3, 입력 필드 밖)
- [x] **그리드 CSV 내보내기 (v13.93)** — DenseGrid ⚙ 메뉴 "⬇ CSV"(보이는 컬럼·정렬 순서, 공용 utils/csv)
- [x] **그리드 내 찾기 (v13.97)** — DenseGrid 🔍/Ctrl+F(그리드 포커스 시 전역 검색 대신 양보) → 보이는 셀 텍스트 부분일치 필터·n/m 카운트·Esc 복원(전 그리드 공용, findable 기본 활성). **E2E: live_g2_grid 필터·무매치0·Esc복원·Ctrl+F 경로**
- [x] **공용 다중행 선택 (v13.97)** — DenseGrid multiSelect(체크박스 열·Shift 범위·전체선택 indeterminate·.msel 스타일, selectedKeys/onSelectionChange). 감사 조회 다중선택→선택 행 CSV 내보내기. **E2E: 전체선택·재클릭 해제·Shift 4행·'선택 CSV(N)' 활성**
- [x] 폼 필드 방향키 이동 ✅ (v34.55 — 셸 전역: .frm 내 텍스트 입력 ↑↓ = 이전/다음 필드 포커스+선택, date/number/checkbox 제외) **[L]**
- [x] ~~페이지네이션/가상화 · 컬럼 리사이즈/순서 · 인라인 편집 · 우클릭 메뉴 · 셀 복사 · 헤더 필터~~ — **stale (2026-07-20 재감사)**: 전 항목 하단 v15.2~v17.5 로 기구현(가상화만 필요 시 후속), g2 스위트 6종이 회귀 감시
- [x] **페이지네이션 (v15.2)** — DenseGrid 150행 초과 시 자동 100행/페이지(pageSize prop 오버라이드)·sticky 하단 페이저(처음/이전/다음/마지막·범위)·키보드 내비 페이지 자동 이동·찾기 변경 시 리셋. 정렬·다중선택(전체)·CSV 는 전체 대상 유지. **E2E: live_g2_paging 8/8(100행/페이지·페이저·select-all 500·재페이지)**. 가상화(virtual scroll)는 필요 시 후속
- [x] **컬럼 리사이즈 · 순서 변경 (v16.1)** — DenseGrid 헤더 우측 핸들 드래그 리사이즈 · 헤더 드래그 순서 변경(prefKey 영속: gridColWidths·gridColOrder) · ⚙ ↺ 컬럼 초기화. **E2E: live_g2_colprefs 6/6(리사이즈·순서·영속·초기화)**. 첫 컬럼 고정은 후속
- [x] **첫 컬럼 고정 (v16.6)** — DenseGrid stickyFirst(첫 데이터 컬럼+체크박스 position:sticky·행 상태별 배경 미러·경계선, 헤더 sticky 좌상단 고정) · 부품 대장 적용. 라이브 확인(th/td position:sticky)
- [x] **행 우클릭 컨텍스트 메뉴 · 셀/행 복사 (v16.5)** — DenseGrid 셀/행(TSV) 클립보드 복사 기본 제공 + `rowActions` prop(행별 커스텀 액션·danger)·부품 대장 선택/삭제 배선. **E2E: live_g2_ctxmenu 7/7**. 인라인 셀 편집은 후속(더블클릭 열기와 상충 → 화면별 opt-in)
- [x] **공용 인라인 셀 편집 (v16.7)** — DenseGrid column.editable + onCellEdit(더블클릭→입력→Enter/blur 커밋·Esc 취소, editValue 초기값·stopPropagation 으로 행 더블클릭과 비충돌) · 제품 코드 마스터 코드명 인라인 수정. **E2E: live_g2_inline_edit 6/6**
- [x] **CAD 오프라인 폴백 문구 정정 (v16.7)** — 오해 소지 'MOCK 모드' → '서버 연결 실패·새로고침(일시적 연결 오류)' (Design Editor·Selection·CAD 뷰어 + seed + bundles 3언어 + 라이브 DB sys_translation 갱신)
- [x] 헤더 컬럼 필터 (v17.5) — DenseGrid `colFilter` + 컬럼 `noFilter` 제외, 고유값>1 열에 깔때기(▽/▼)·Excel autofilter식 값 체크리스트 팝업(검색·전체/해제·적용, position:fixed)·다열 AND·찾기 병행·페이지 리셋. 적용: 감사 조회·거래처·발주·부품 대장. **E2E: live_g11_col_filter 7/7(깔때기·팝업·값 선택 적용→행 필터·활성 ▼·전체 선택 해제 복원)**
- [x] 그리드 빈 상태/로딩 상태 표준 (v17.9) — DenseGrid `loading`(boolean/문자열)·`emptyText` props → 헤더 유지 + 중앙 표준 표시(📭 빈 상태·dg-spin 스피너 로딩), 각 화면 임시 div 대체(변경 이력 대장·제품 코드 마스터·데이터 번역). **E2E: live_g15_grid_states 4/4(data-grid-empty·📭·표준 문구·헤더 유지)** **[M]**
- [x] ~~`Ctrl+P` 명시 바인딩~~(브라우저 네이티브 인쇄 + 파일>인쇄 메뉴로 기커버 — 판정) · F5 재조회 ✅ (v34.55 — 셸이 F5 를 F8 로 별칭 디스패치: 기존엔 새로고침만 차단하는 죽은 키) · 폼 필드 방향키 이동 ✅ (v34.55 상단 참조) **[L]**

### G3. 도메인 확장 (신규 갭) 〔업무 사이클 심화〕
- [x] **PO 라이프사이클 심화(ERP-017) (v13.95)** — **alembic 0014**(erp_po + erp_po_item, C6 12번째 증분 마이그레이션 0013→0014 라이브 자동적용). 구조화 PO(헤더+라인) 생성·목록·상세 · 승인(DRAFT→APPROVED) · 입고(GR: received_qty 누적, **발주 초과 409=3-way 수량 match**, inv_movement IN 기록, 전 라인 완료→CLOSED). 신규 화면 `발주 (G-3)`(ERP>Purchasing). **E2E: 생성→미승인입고 409→승인→부분입고 RECEIVING→초과입고 409→전량입고 CLOSED(100%·matched)·CASCADE 검증**
- [x] **BOM 비교/diff (v13.96)** — `GET /codes/bom-compare?base=&target=`(두 코드 구성 diff: 추가/삭제/수량변경/동일, code_relationship 기반). 신규 화면 `BOM 비교 (G-3B)`(PLM). **E2E: KDCR3-13 vs KAD900FW 추가3·삭제5·self identical·미존재 404**. (Rev 스냅샷 부재로 코드 간 비교로 제공 — 버전별 BOM 스냅샷은 후속)
- [x] **재고 예약/할당(ATP) (v14.7)** — **alembic 0015**(inv_reservation) + 예약(POST /erp/stock/reserve, 가용 초과 409)·해제(release)·목록·ATP 조회(보유−ACTIVE 예약=가용) + Stock Check(pr-items) 가용재고 기반 판정 + 재고 화면 ATP/예약 패널·예약/해제. **E2E: live_g3_atp 11/11(예약·가용−1·409·목록·해제 복원·pr-items available)**
- [x] **공급처 평가/등급 (v14.8)** — **alembic 0016**(com_supplier_eval) + 발주 이행 지표(GET /erp/suppliers/{id}/metrics: PO 건수·발주/입고·이행률→납기 제안)·평가 등록/갱신(가중 총점 납기.4·품질.4·단가.2→A/B/C/D, 최신 등급 com_company 반영)·목록 + 업체 대장 스코어카드 패널(지표·자동산출·저장·이력). **E2E: live_g3_supplier_eval 14/14(총점·등급 경계·upsert·마스터 반영·422·psql CASCADE 정리)**
- [x] **Lot/Serial 추적 (v17.0)** — **alembic 0020**(inv_movement.lot_no·serial_no + 인덱스) + 입고 시 로트/시리얼 적재(POST /erp/stock/inbound lotNo/serialNo)·로트별 잔량 산출(GET /erp/stock/lots: 로트·시리얼별 IN−OUT 집계·위치, 이력 기반)·이력 추적 genealogy(GET /erp/stock/trace lot|serial, 둘 다 미지정 422)·입출고 이력 lot/serial 노출 + 재고 화면 입고폼 Lot/SN 입력·이력 Lot/SN 컬럼·Lot/Serial 추적 패널(로트 잔량 그리드+선택 시 genealogy). **E2E: live_g7_lot_serial 17/17(L1 5+3=8 집계·L2·Serial 잔량·추적 시간순·422·movements lot 노출·psql 정리)** **[L]**
- [x] **X-code 검토 UI (v15.0)** — GET /cpq/x-review(PENDING 비표준 X-code 견적안 대기열)·POST /cpq/selections/{id}/x-review(APPROVE/REJECT→x_code_status, 요청자 알림·X_CODE_REVIEW 감사, 비-PENDING 409·잘못된 결정 422) + X-code 검토 화면(C-1X, CPQ>Product>AHU) 승인/반려·검토의견. **E2E: live_g3_xcode 10/10(대기열·승인/반려·표준 제외·재검토 409·422·정리)**
- [x] **product_code 마스터 CRUD (v15.1)** — GET/POST/PATCH/DELETE /codes/products(수동 생성 DRAFT·코드명/상태 DRAFT·APPROVED·INACTIVE(비활성)·중복 409·그룹없음 422·참조 가드 삭제 409) + 제품 코드 마스터 화면(M-3-8, Code Set-up>Product Code) 등록/승인/비활성/복원/삭제·상태 필터. **E2E: live_g3_product_master 12/12**
- [x] **마스터 수정 갭 — 프로젝트 메타 + 거래처 비활성 (v16.9)** — PATCH /projects/{no} 확장(sales_stage 단독→ projectName·client(고객사 자동생성/해제)·dueDate 동시 수정, D9 낙관적 잠금 유지·PROJECT_UPDATE 이력·빈명/잘못된 납기 422)·GET /projects dueDate 추가 + **alembic 0019**(com_company.is_active) PUT /companies active 소프트 비활성/재활성·GET /companies?active_only 선택리스트 필터·isActive 반환 + FE ProjectScreen ✎수정(명/고객/납기·409/422 전파)·Project 패널 명/납기 표기·CompanyMaster 상태 컬럼·수정 다이얼로그 활성/비활성·QuickEditDialog placeholder. **E2E: live_g6_master_edit 19/19(명/고객/납기 반영·409 stale·422·비활성→active_only 제외→재활성 복귀·psql 정리)**. 잔여: 단가 값 수정(현재 close/supersede 패턴 의도적)·code_relationship 수정·도면 메타 수정은 후속 **[S]**
- [x] **역할 생성/삭제 (v16.0)** — POST /roles(커스텀·예약(내장4)/중복 409)·DELETE /roles/{name}(내장 삭제 409·사용자 배정 시 409·권한 매트릭스 연쇄 정리, ADMIN) + 권한 매트릭스 화면 ＋역할·비내장 역할 헤더 ✕ 삭제. **E2E: live_g3_roles 10/10**
- [x] **Report Center (v15.3)** — GET /reports/catalog(리포트 카탈로그+건수: PCR·견적·문서·CAD·감사)·GET /reports/pcr/{id}.pdf(**PCR 수익성 보고서 RPT-07**, reportlab CID 한글: 기여마진·EBIT·원가 구성) + Report Center 화면(RPT, CPQ>Document) 카테고리 카드·화면 이동·PCR PDF 생성. **E2E: live_g3_report_center 9/9(카탈로그·PCR PDF·404·UI)**. 스케줄링은 후속
- [x] **다통화/환율 마스터 + 세금엔진 (v15.8·v16.2)** — **alembic 0018**(fx_rate·tax_code) + 환율 CRUD(KRW 기준·통화별 최신·KRW 등록 422)·세금코드 CRUD(중복 409·세율 422)·세금엔진(POST /finance/quote-calc: 통화 금액→세액·합계 + 기준통화 KRW 환산·미등록 통화 422) + 다통화·세금 마스터 화면(M-13-1, ERP) 환율/세금 관리·세액 계산기. **E2E: live_g3_finance 13/13**. **v16.2 견적 연결**: 견적 확정(POST /cost/quotations)에 currency·taxCode → PCR 매출 통화 환산·세액 적재·세액포함 total 저장·목록 breakdown + Run 화면 통화·세금 콤보·세액 표기. **E2E: live_quote_tax 10/10**. **v16.3 견적서 PDF**: render.pdf 통화·공급가액·세액(%)·합계 요약 블록(KRW=천원·외화=통화 표기). **E2E: live_quote_pdf_tax 6/6**
- [x] **핵심 마스터 대량 import (v15.9)** — POST /companies/import-excel(업체명·유형·국가·결제조건·중복 거부)·/parts/import-excel(부품번호·부품명·사양·단위·중량·공급처·중복 거부·공급처 자동생성) + 거래처/부품 대장 ⬆ 대량등록(등록·거부 리포트, 헤더 누락 422). **E2E: live_g3_bulk_import 7/7**. 도면·BOM import 는 후속
- [x] **근무일/휴일 캘린더 (v15.6)** — **alembic 0017**(cal_holiday) + 공휴일 CRUD(중복 409)·영업일 계산(GET /calendar/workdays 주말·공휴일 제외·/calendar/due N영업일 후 납기) + 마일스톤 목록에 **workdaysLeft**(영업일 잔여) + 근무일·휴일 캘린더 화면(M-8-6, ERP) 등록/삭제·기한 계산기. **E2E: live_g3_calendar 9/9(영업일 4→3·납기 08-11·중복 409·422·workdaysLeft)**
- [x] **데이터 콘텐츠 i18n (v17.3)** — 마스터 데이터 번역 트랙 확립(sys_translation 재사용, 마이그레이션 불요): entity_type COMPANY/PRODUCT/DOCUMENT·entity_id=원본PK·field=name/codeName/title + GET /i18n/data/{type}(원문+번역)·/map(오버레이 맵)·PUT /i18n/data(upsert·빈값 삭제, 미지원 type 404·locale 422) + 데이터 번역 화면(M-13-2, Code>Product Code) 대상×로케일 인라인 편집·번역률 + 거래처 대장 비-KO 로케일 업체명 오버레이(원문 병기) + /i18n/data* 인증 가드(미들웨어 공개 예외를 번들 경로로 한정). **E2E: live_g10_data_i18n 15/15(upsert·목록/맵 반영·원문 보존·로케일 독립·PRODUCT·빈값 삭제 자기정리)** **[M]**

### G4. 잔존 무반응·하드코딩 (현재 트리 실재 확인) — 🏁 **완료 (v13.91)**
- [x] `RunScreen` 산출물 `ZIP ⬇`(→/files/zip)·`폴더 열기`(→Project Folder) 실배선 + 제목 activeProject 반영
- [x] `WorkProcessScreen` "Table" `＋ ✎ ⬇` → 실 CSV 내보내기(공용 `utils/csv.ts`)
- [x] ~~잔여: `ProjectScreen` 샘플 행·`EventDetailScreen` `'PS-612'` 폴백·데모 재배정~~ — **무효 (2026-07-20)**: 레거시 SPA 중단(v33.1, /edim-static 410 봉인)으로 해당 화면 자체가 퇴역, Next 메인 콘솔은 실데이터

### G5. UI 폴리시 (사용자 피드백) — 🏁 **완료 (v13.98·v14.9·v15.7)**
- [x] **초기 로딩 페인트 (v15.7)** — SPA 부트 스플래시(#root 인라인, React 마운트 시 교체)로 빈 화면 제거 + 폰트 CDN preconnect·theme-color. **SSR 검토 결과 미채택**: auth-gated 내부 SPA·Python(FastAPI) 백엔드 특성상 SEO 무관·서버 프리페치 불가(클라 토큰)·Node 런타임 이중화 비용 → 실질 이득인 초기 페인트만 저비용 반영
- [x] **그리드 오버플로우/이중 스크롤 (v14.9)** — DenseGrid ⚙ 컬럼 메뉴 `position:fixed`(GroupBox `.gc overflow:auto` 클리핑 회피·트리거 앵커) + 감사 조회 이중 세로 스크롤 제거(외곽 overflow:auto→hidden·그리드 flex:1·상세 maxHeight 40%). **E2E: live_g5_ui_overflow 4/4**
- [x] **CAD 텍스트 grab 방지** — CAD 캔버스·이동 블록 드래그 시 텍스트 선택 방지(`.cvs`·`[data-cad-wrap]`·`[data-cad-svg]`·`.m2` user-select:none). **라이브 검증: computed user-select=none**
- [x] **모달 오버플로우 수정** — Data 위치 지정 바인딩 모달 필드가 박스 밖으로 넘침 → `.frm c2` 적용 + `.frm` 컬럼 `minmax(0,1fr)`(1fr 의 min-content 로 인한 필드 오버플로우 근본 차단, 전 narrow 모달 적용). **라이브 검증: 가로 오버플로우 0px·필드 박스 내**

> 우선순위 표기 [H]high·[M]med·[S]small·[L]low. G1(캔버스 편집)·G2(그리드 UX)가 사용성 체감 최대. G3 은 제품 심화(중규모+). G4 는 소규모 마감.
