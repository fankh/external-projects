...
- [ ] **gen_i18n_bundles.py 원천 수렴** (F1 구현 중 발견): 생성기는 `UI_TRANSLATIONS*` 딕셔너리만 병합하는데 B13~B16 키는 시드 인라인 INSERT(TAB_LABELS_V14 등)로 들어가 bundles.ts 재생성 시 유실됨 — 시드 전 키를 생성기가 읽도록 통일 (그 전까지 bundles.ts 직접 편집이 실질 관행)
...
### F9. 다이얼로그 UX 표준
- [ ] 등록 다이얼로그 7종(`data-*-reg`) **Escape 로 닫히지 않음** 실측 (백드롭 클릭 닫기는 있음 — F1 구현 중 확인·정정). 공통 Dialog 래퍼: Escape 닫기(더티 시 확인)·초기 포커스·포커스 트랩
