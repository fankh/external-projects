...
### C11. 셸·상세 화면 잔여 실데이터화
- [ ] 상태바 "승인 대기 4" 하드코딩 → 승인함 실카운트 (로그인 시 + 결정/요청 후 갱신)
- [ ] 타이틀바 "Micron #7 (Pre-Sales)" 컨텍스트 → 활성 프로젝트(prj_project) 기반 표시
- [ ] 문서 상세(OutputDocScreen) 승인 요청 — 로컬 stage 증가만 → PATCH /documents/{no}/status 전이 + 범용 승인 요청 배선 (B21 API 기활성)
- [ ] M-14-6 감사 로그 박스 — AUDIT_LOG mock → GET /history 실데이터 (계정 작업 직후 갱신)
- [ ] 부품 상세 — PART_INFO/DWG_DIMS/PROCESS_DEF mock → 치수(도면 치수 필터)·Work Process 실데이터, "Block 편집" 버튼 = Design Editor 탭 직행
- [ ] C-1 캔버스 AHU_BLOCKS·"전체 47" — arrangement_component 실블록 + 실카운트

### C12. i18n 완전판 + 체커 확장
- [ ] 신규 화면 내부 라벨 t() 적용 + 시드 키: Hierarchy 편집 UI·CompanyMaster 그리드/다이얼로그·PartDetail·ArrangementSetup 헤더·Warehouse 다이얼로그 타이틀
- [ ] check_i18n_en 대상 24→33 화면 확장 (B13~B19 신규 화면 포함)
- [ ] e2e_fallback 폴백 커버 신규 화면 추가 (현재 24화면 — plm-arr/quality/parts/material/code-variant/hierarchy/company/warehouse/templet 미커버)

### C13. 문서 파이프라인 개선 (감사 권고)
- [ ] FVT 확정 행 freeze 플래그 (재생성 덮어쓰기 방지 — make_fvt_xlsx.py)
- [ ] RTM C-단계 확장 규칙 (C 배치 구현 시 요구 추적 행 추가)
- [ ] 포털 항목 해시/버전 표기 (문서 변경 감지)
...
### F1. 프로젝트 도메인 실체화 — S-3-5 가 단일 mock 프로젝트
현재 화면 전체가 상수 `PS-61313-5` 고정: 목록·신규 등록·채번 전부 없음 (projectService = get/setStage 뿐).
- [ ] prj_project 목록 + 신규 등록 (PS 자동 채번 — 화면 라벨 "자동 채번 (PS-)" 이 이미 약속) + 프로젝트 선택 → 화면 컨텍스트 전환
- [ ] 접수 자료 업로드 실배선 — 현재 **로컬 휘발**인데 상태바는 "Project Folder RECEIVED 저장" 이라고 주장 (라이브 실측: 쓰기 0 · 새로고침 후 소실). fileService.upload(RECEIVED) 실호출 + Folder 연동
- [ ] 타이틀바 활성 프로젝트(C11)와 연계 — 여기서 선택한 프로젝트가 컨텍스트

### F2. 사용자 등록·수정 완결 — M-14-6
- [ ] '＋ 사용자 등록' 버튼 = **onClick 자체가 없음** (실측 무반응). 등록 다이얼로그(login·이름·부서·레벨·초기 비밀번호 정책) + `POST /users` + USER_CREATE 감사 — 현재 신규 계정은 시드로만 존재
- [ ] 사용자 정보 수정(이름·부서·이메일) PATCH — unlock/level/역할만 있고 프로필 수정 없음

### F3. 권한 기반 UI 게이팅 — SYS-005 (화면 스스로 명시한 원칙: "권한 없는 메뉴는 미표시")
GENERAL(kim01) 실측: 모든 쓰기 버튼 노출·활성 → 다이얼로그 작성까지 진행 후 `POST /parts→403`. 서버 RBAC 은 정상, UI 만 무권한.
- [ ] `GET /auth/permissions`(B21 기구현)를 프론트 셸이 로그인 시 소비 → 쓰기 버튼 disabled(사유 툴팁)·관리 화면(M-14-6 등) 메뉴 미표시
- [ ] GENERAL 이 M-14-6 진입 시 감사 로그 박스·권한 매트릭스가 그대로 보임 — 화면 진입 가드 (403 안내 화면)
- [ ] 승인함: 결정 권한 없는 사용자에게 승인/반려 버튼 활성 — 읽기 전용 표시 + 내 요청 필터

### F4. 잔존 무반응·메시지-온리 일소 — "no-op 0" 재감사 누락분
전부 라이브 네트워크 관측으로 확인 (클릭 시 쓰기 호출 0):
- [ ] S-1-4 Code Relationship '승인 요청' — **메시지-온리** (B1 누락). POST /approvals(code_relationship) + 승인 시 관계 APPROVED 전이
- [ ] S-1-1: '저장 F12' 버튼·'＋ 값 추가' onClick 없음 · '조회'/F8 = 메시지-온리(재조회 없음)
- [ ] M-3-7 데이터 Table: '✎ 편집'·'⬆ Export' onClick 없음 (Export 는 D8 XLSX 전면과 연계)
- [ ] S-3-4 Print Set-up: '기본 양식 배치'·'Data 호출'·'그래프 불러오기'·'Printer/PDF/Office'(6버튼) onClick 없음 · 'Data 위치 지정' 메시지-온리 — 양식 편집기 실동작(레이아웃 JSON = tbx_templet 연계) 또는 정직한 범위 표기
- [ ] 코드 상세 헤더 'Variants'·'Referencers' onClick 없음 (셸 툴바는 동작 — 동선 통일)
- [ ] M-5-4 문서함 F8 = 메시지-온리 — 실재조회로 (타 화면 F8 실동작 표준과 불일치)
- [ ] 셸 툴바 'Referencers' = **KDCR 3-13 고정** (라이브 실측: ERP 컨텍스트에서도 KDCR 3-13 조회 3건 발사) — 활성 탭 컨텍스트 기반으로

### F5. 마스터 데이터 수정·정정 전면 — 등록만 있고 Update 가 없다
서비스 계층 실측: create 만 존재, update 부재. 잘못 입력하면 정정 불가(재등록도 중복 409).
- [ ] 공급처(M-14-2): 행 더블클릭 무반응 실측 — 수정 다이얼로그(PUT /companies/{id}) + 비활성(거래중지) 플래그
- [ ] 부품(M-4-7): 속성 수정 (PUT /parts/{no}) — 현재 등록·삭제만
- [ ] 재질(M-3-2)·검증 규칙(M-4-5)·Variant 값(S-1-2): 수정·폐기(deprecate)
- [ ] 창고(M-8-4): 노드 개명·속성(위험물·검사주기) 수정 — 현재 등록·삭제만
- [ ] 단가(M-12-5): 적용 종료일 마감(정정) PATCH — 행 더블클릭은 코드 상세로만 이동 (수정 동선 없음 실측)
- [ ] 문서 메타(제목·유형·Grade) 수정 · Templet(S-2-3)/Macro(S-2-2) 삭제(참조 보호 409) — Studio F3 = "해당 동작이 없습니다" 실측
- [ ] Arrangement(M-4-2): 구성품 삭제·수량 수정

### F6. 통합 검색 커버리지 확장 — ⌘K
라이브 실측: 'Impeller'(부품)·'공조'(공급처) → **결과 0** (드롭다운 자체 미표시), 'KDP' → 코드만.
- [ ] GET /search 그룹 확장: 부품(prt_part)·공급처(com_company)·창고(erp_warehouse)·매크로(tbx_macro)·프로젝트(prj_project)·사용자(ADMIN 한정)
- [ ] 그룹별 클릭 = 해당 화면 딥링크 (부품=부품 상세 · 공급처=M-14-2 · 매크로=Studio 로드)

### F7. 이력 diff 뷰어 — M-15-9
- [ ] diff 칩 = 메시지-온리 실측 ("diff — sys_user #3 before/after JSON 비교" 문구만). sys_history before/after JSON 을 나란히 비교하는 모달 (변경 필드 하이라이트)

### F8. 그리드 정렬 — DenseGrid 공통
- [ ] 전 화면 헤더 클릭 정렬 없음 실측 (단가 대장에서 확인 — DenseGrid 에 onClick 자체 부재). 공통 클라이언트 정렬(▲▼ 토글) + 대량 대장(단가·이력·문서)은 서버 정렬 파라미터
- [ ] (컬럼 표시 설정·XLSX export 는 D8 트랙 — 정렬은 선행 소품)

### F9. 다이얼로그 UX 표준
- [ ] 등록 다이얼로그 7종(`data-*-reg`) **Escape 로 닫히지 않음** 실측 (백드롭 클릭도 무시 — 모듈 내비까지 가림). 공통 Dialog 래퍼: Escape·백드롭 닫기(더티 시 확인)·초기 포커스·포커스 트랩
- [ ] 다이얼로그 열림 중 뒤 화면 상호작용 차단 상태를 시각적으로 명확히 (현재 오버레이 투명)

### F10. 조회 UX 소품
- [ ] MDI 탭 12+ 열면 폭 압축으로 제목 판독 불가 (스크린샷 실측) — 탭 최소폭 + 가로 스크롤 or 오버플로 '▾ 목록' (창 메뉴 목록 재사용)
- [ ] Dashboard KPI 카드 클릭 무반응 실측 — 카드별 드릴다운(승인함·업무함·단가 등 해당 탭)
- [ ] 승인함 유형·상태 필터 (현재 검색 input 1개뿐 — D8 일괄 결정과 연계)
