...
| 화면 | mock 직접 렌더 | 실 소스(기존재) |
|---|---|---|
| **부품 상세** (PartDetailScreen) | `PART_INFO`(부품정보)·`DWG_DIMS`(치수 필터)·`PROCESS_DEF`(Process/Workplace/인원/WTime) 전부 mock. BOM 만 실호출인데 `partService.bom('KDCR 3-13')` **코드 고정** | drawingLedgerService(치수)·partService.bom(선택 부품) |
| **문서 상세** (OutputDocScreen) | '승인 요청' = 로컬 `stageIdx+1` 만(영속 없음) · `DOC_HIST`(문서 이력) mock 렌더 | `PATCH /documents/{no}/status`(B21)·`GET /history` |
| **코드 상세** (CodeDetailScreen) | 단가 이력 섹션 = `PRICES` mock 필터(실fetch 없음). ※Referencers·승인이력·슬롯·도면은 실호출 확인 | `priceService`(cst_price 실단가) |
| **Work Process** (WorkProcessScreen) | 자재행 `MATERIAL_ROWS` mock(MAKE/BUY 만 실저장 병합)·코드 `KDCR 3-13` 고정 | partService.bom / 자재 소요 소스 |

- [ ] 부품 상세 — 선택 부품 기준 치수·공정·BOM 실조회 (코드 고정 제거) · [ ] 문서 상세 승인 요청 = 상태 전이 PATCH 영속(**C11 항목**) + DOC_HIST 실history
- [ ] 코드 상세 단가 이력 = priceService 실단가 · [ ] Work Process 자재행 실소스 + 코드 컨텍스트
- 규모 S~M(대부분 기존 엔드포인트 배선). ※**EventDetail 전후공정 mock 은 B6 에서 시연용 의도 유지 — 제외**(설계 결정).
