...
- [x] 헤더 컬럼 필터 (v17.5) — DenseGrid `colFilter` + 컬럼 `noFilter` 제외, 고유값>1 열에 깔때기(▽/▼)·Excel autofilter식 값 체크리스트 팝업(검색·전체/해제·적용, position:fixed)·다열 AND·찾기 병행·페이지 리셋. 적용: 감사 조회·거래처·발주·부품 대장. **E2E: live_g11_col_filter 7/7(깔때기·팝업·값 선택 적용→행 필터·활성 ▼·전체 선택 해제 복원)**. 잔여: 그리드 빈 상태/로딩 상태 표준 **[M]**
