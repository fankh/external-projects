...
- [x] 실적→PCR 반영 재계산 (v18.2) — GET /cost/pcr/{id}/actual: PCR 프로젝트 귀속 실적(cst_actual) 합계로 직접비 치환(매출 고정)→기여마진·EBIT 재산출 + 추정 대비 차이(마진율 델타), 실적 없으면 actualAvailable=false·404 + 원가실적 화면 '실적 반영 PCR' 패널(추정 vs 실적 매출·직접비·기여마진·EBIT·마진율). **E2E: live_g18_pcr_actual 13/13(구조·항등식·실적 적재 반영·직접비/마진/EBIT/차이 재계산·404)**. 잔여: C3 Dashboard 차이 위젯
