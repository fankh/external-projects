...
| **부품 상세** (PartDetailScreen) | `PART_INFO`·`DWG_DIMS`·`PROCESS_DEF` 전부 mock. BOM 만 실호출인데 `partService.bom('KDCR 3-13')` 코드 고정 | ⚠ **백엔드 선행** — 부품 상세(정보+치수+공정)를 partId 로 반환하는 엔드포인트 부재. 캔버스 Block↔부품 매핑 설계 필요 |
| **문서 상세** (OutputDocScreen) | '승인 요청' = 로컬 `stageIdx+1` 만(영속 없음, **C11 항목**) · `DOC_HIST` mock | ⚠ **백엔드 선행** — `docNo` 는 신규 채번(allocate)일 뿐 doc_control 행 미존재 → `PATCH .../status` 는 404. Run 산출물을 doc_control 로 **등록하는 플로우**(POST /documents)가 선행돼야 상태 전이 영속 가능 |
| **Work Process** (WorkProcessScreen) | 자재행 `MATERIAL_ROWS` mock(MAKE/BUY 만 실저장)·코드 `KDCR 3-13` 고정 | ⚠ **백엔드 선행** — 공정별 자재행(BOM×공정) 반환 소스 부재 |

- 결론: 4건 중 **코드 상세 단가만 순수 배선으로 해소**(완료). 나머지 3건은 "실 엔드포인트 기존재 배선만"이 아니라 **신규 엔드포인트/등록 플로우 선행 필요** — 실측으로 정정. C 트랙(운영·제품 완결)과 함께 진행.
