...
| **부품 상세** (PartDetailScreen) | `PART_INFO`·`DWG_DIMS`·`PROCESS_DEF` 전부 mock. BOM 만 실호출인데 `partService.bom('KDCR 3-13')` 코드 고정 | 🔶 **C-트랙(중)** — 진입은 Design Editor 캔버스 블록(dwg_document 실데이터, B16)이나 블록 payload 에 `part_id` 없음 → 블록→부품 해소 + prt_part·dwg_bom·dwg_dimension·erp_work_process 4테이블 집계 엔드포인트 필요. 이름 매칭 취약 → 설계 결정 선행 |
...
| **Work Process** (WorkProcessScreen) | 자재행 `MATERIAL_ROWS` mock·일부 행 JSX 하드코딩·코드 `KDCR 3-13` 고정 | 🔶 **C-트랙(대)** — 화면이 product_code 컨텍스트를 받지 않음(전역 make/buy 저장만). 코드 선택 + BOM 바인딩(UX+백엔드) 재설계 필요 |

- 결론: 4건 중 **코드 상세 단가(v13.18)·문서 상세 승인 영속(v13.19) 2건 해소**. 잔여 2건은 "엔드포인트 추가"가 아니라 **화면 재설계(집계 엔드포인트+블록↔부품 해소 / 코드 컨텍스트+BOM 바인딩)** — 실측으로 정정, **C 트랙 정식 작업**으로 이관(취약 heuristic 회피).
