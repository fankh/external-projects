...
- [x] 재고 단가(STOCK) 자동 적재 연계 (v17.8) — **alembic 0022**(inv_stock.unit_price) + 입고 단가 지정 또는 cst_price(STOCK, 유효기간·item_code→part_no/product main_code 매칭) 자동 적재 → 이동평균 단가 갱신·평가액(수량×단가)·음수 422 + /erp/stock unitPrice/value·inbound 응답 avgPrice/priceAuto/value + 재고 화면 입고 단가 입력·단가/평가액 컬럼·총 평가액. **E2E: live_g14_stock_valuation 11/11(이동평균 100→150·자동 1500·평가액·422·목록 반영·psql 정리)**. 잔여: Mobile '입고처리' 실배선(프리뷰→QR I-006)
