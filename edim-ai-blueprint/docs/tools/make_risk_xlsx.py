...
RISKS = [
 ...
 ("R-07", "기술", "DWG 변환 손실·ODA 라이선스 미확정 — DWG 경로는 501 대기 상태",
  "보완노트 §3.3", "중", "중", "완화",
  "플러그블 구조 완료(경로 env 만 설정하면 활성) — 라이선스 확보 전까지 501 정직 응답. "
  "미구현 목록 잔여 1건이 이 항목(2026-07-25 기준 유일한 코드 잔여)", "아키텍트", "Open", "2026-07-11"),
 ...
 ("R-11", "기술", "성능 목표 미충족 위험 (REQ-N-001~005: 산출물 1시간·BOM 30초·응답 2/3초)",
  "요구사항정의서", "중", "하", "완화",
  "**종결(2026-07-25)** — 실측 BOM 전개 46.5req/s·p95 138ms(목표 30s 대비 여유), 핵심 쿼리 전량 인덱스 백업 EXPLAIN 확인. "
  "상설 감시: tests/load_baseline.py. 고객 실데이터 규모 확보 시 재측정", "QA·인프라", "종결", "2026-07-11"),
 ...
 ("R-15", "외부", "AI 기능 활성화가 외부 크레딧에 종속 — 충전·조직 설정 문제로 합성 기능 사용 불가 상태 지속 가능",
  "C9 진단 이력(미구현목록)", "중", "중", "완화",
  "**설계로 완화됨** — 크레딧 없어도 검색 근거·샘플 초안 제공하고 합성만 사유와 함께 보류(live_ai_degrade 6체크 상설 감시). "
  "해제 시 코드 변경 없이 자동 전환. 트리거: 크레딧 차단 지속 시 조직·워크스페이스 한도 확인 후 지원 문의", "PM·인프라", "Open", "2026-07-25"),
 ("R-16", "운영", "배포 중단이 로그 없이 발생 — systemd TimeoutStartSec 초과 시 빌드가 조용히 종료",
  "9.53 실측(autodeploy)", "중", "하", "회피",
  "**해소(2026-07-25)** — TimeoutStartSec 900→2400 상향 적용·설치배포매뉴얼 명문화. "
  "부수 발견: 배포 확인 시 journalctl 시간 필터 누락하면 과거 실패를 오탐 → 매뉴얼에 --since 필수 기재", "인프라", "종결", "2026-07-25"),
