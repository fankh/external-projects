...
def main():
    ...
    info = [
        ("문서 버전", "v0.2 (자동 생성)"),
        ("작성일", "2026-07-07 (v0.2 갱신 2026-07-15)"),
        ("생성 방법", "make_rtm_xlsx.py — 요구사항정의서·기능정의서·메뉴정의서 xlsx 조인 (수정 시 재생성)"),
        ("추적 체인", "요구사항 → 기능 → 메뉴 → 화면 → 컴포넌트 → DB → Phase"),
        ("추적 범위", "본 RTM 은 요구사항(REQ-F/NFR) 중심 추적만 담당한다. 요구사항 확정 이후의 구현 증분"
                    "(B/C/D/E/F/G 배치)은 docs/EDIM_미구현기능목록.md 가 배치 단위(구현·검증·버전)로 추적하며"
                    " RTM 형식을 확장하지 않는다 (C13 규칙, 2026-07-15 명문화)."),
