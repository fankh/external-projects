...
FUNC = [
 ...
 ("RCCS 코드", [
  ("REQ-F-007", "Sub Code 등록", "코드 그룹·자릿수(Slot)·값 등록, 중복검토, Table 참조 값, Excel Import/Export, 코드별 DWG/자료 연결", "필수", "P1", "CODE-001~005/015/016", ""),
 ...
 ("Toolbox", [
  ("REQ-F-019", "UI Designer", "위젯 팔레트 Drag&Drop 폼 빌더, 동작 Templet 바인딩, 버전·승인·게시, 동적 렌더링", "필수", "P4", "TBX-001~003/013", ""),
 ...
 ("원가·견적", [
  ...
  ("REQ-F-032", "원가 계산", "자재비(면적·무게·부피×단가)·제조비(시간·임율·장비)·직접비 집계", "필수", "P3", "CST-002~004", ""),
 ...
 ("ERP", [
  ...
  ("REQ-F-036", "프로세스 상태기계", "프로세스 정의(40종 코드) Set-up(선행·후행·기한·자동), 이벤트 상태 전이, 부서별 업무함", "필수", "P5", "ERP-002/003", "슬라이드 17 5-1~5-3"),
  ("REQ-F-037", "영업 프로세스", "견적검토(PCR)→견적(QCR)→수주(OR)→승인도서(AP/APP)→출고·납품(FDR/SFO/DF)→기성(IR)", "필수", "P5", "ERP-004/005/009", ""),
  ("REQ-F-038", "구매·자재", "구매 조건·공급자 코드 매핑, 발주(PR/PO)·입출고(MI/MO/RM), 재고·창고(위험물성·위치계층), MRP, 일반발주", "필수", "P5", "ERP-007/017~022/026/029", "슬라이드 46"),
  ("REQ-F-039", "생산", "생산계획(MP)·Scheduling·Capacity, 작업지시(WR), 공정 관리(실시간 수집), 반·완성품, 외주·재작업", "필수", "P5", "ERP-006/008/023~025/030", ""),
