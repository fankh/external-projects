...
# (그룹, 파일 상대경로, 표시명, 설명)
FILES = [
 ("기준·관리", "pdf/EDIM_시스템개요서.pdf", "시스템 개요서", "발표자료 78슬라이드 분석 — 전 문서의 기준 (다이어그램 8)"),
 ("기준·관리", "pdf/EDIM_문서관리요약.pdf", "문서 관리 요약", "문서 지도·추적 체계·관리 규칙 — 단일 진입점"),
 ("기준·관리", "EDIM_산출물목록.xlsx", "산출물목록", "37종 레지스터 — 상태·작업자·우선 권고"),
 ("기준·관리", "pdf/EDIM_요구사항보완노트.pdf", "요구사항 보완노트", "PPT 재검토 발견·고객 협의 필요 항목"),
 ("기준·관리", "pdf/EDIM_시연시나리오.pdf", "시연 시나리오", "10분 표준 데모 절차 — 단계별 조작·멘트·질의응답·트러블슈팅"),
 ("기준·관리", "pdf/EDIM_미구현기능목록.pdf", "미구현 기능 목록", "전수 감사 — 구현 배치 B1~B15 · 외부 의존 · 제외 항목 (단계별 실행 계획)"),
 ...
 ("DB·스키마", "pdf/EDIM_DB정의서.pdf", "DB 정의서 (PDF)", "54테이블 462컬럼 — 설계 원칙·제약·리뷰 이력 v0.5"),
 ...
 ("설계", "pdf/EDIM_컴포넌트정의서.pdf", "컴포넌트 정의서 (PDF)", "39컴포넌트 — 아키텍처 결정·구축 현황"),
 ("설계", "EDIM_컴포넌트정의서.xlsx", "컴포넌트 정의서 (Excel)", "컴포넌트 목록 + API 108"),
 ("설계", "api/edim-openapi.yaml", "OpenAPI 3.1 스펙", "107 오퍼레이션 · 52 스키마 — 검증 통과 (자동 생성)"),
 ("설계", "pdf/EDIM_인터페이스정의서.pdf", "인터페이스 정의서", "공통 규약·오류 카탈로그·외부 연계 8종"),
 ("설계", "pdf/EDIM_클래스정의서.pdf", "클래스 정의서", "언어 중립 도메인 모델 — 11도메인·인바리언트 대응표"),
 ("설계", "EDIM_권한승인정의서.xlsx", "권한·승인 정의서", "권한 매트릭스(98메뉴)·승인 상태기계 13·Grade"),
 ("설계", "pdf/EDIM_개발표준정의서.pdf", "개발 표준 정의서", "명명·API 규약·FE/BE·Git·테스트·보안·i18n"),
 ("설계", "pdf/EDIM_배치보고서양식정의서.pdf", "배치·보고서양식 정의서", "배치 잡 JOB-01~08·운영 배치 + Print Form 통제·양식 카탈로그 RPT/FORM"),
 ...
 ("계획·이행", "pdf/EDIM_사업수행계획서.pdf", "사업수행계획서", "범위·조직·일정·관리 체계 — 계약 확정 시 v1.0 승격"),
 ("계획·이행", "04_WBS/EDIM_WBS.xlsx", "WBS·일정표", "38 Task · 44주 간트 · 마일스톤 4 (시작일 가정)"),
 ("계획·이행", "EDIM_위험관리대장.xlsx", "위험관리대장", "초기 위험 14건 — 3×3 평가·대응 전략, 주간 갱신"),
 ...
 ("매뉴얼", "pdf/EDIM_설치배포매뉴얼.pdf", "설치·배포 매뉴얼", "Self-managed 단일 서버 절차·검증 체크리스트·업그레이드/롤백·운영 확장"),
