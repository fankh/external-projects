...
| **P4-3** 🔶 DXF 완료 | CAD 호환 — **DXF 뷰어**(`/cad/view` ezdxf→정규화 문서→SVG, 레이어 토글·줌)·**Import**(`/cad/import` 업로드+파싱+Folder 등록)·**Export**(`/cad/export-dxf` 엔진 치수 반영). 진입점: Run 산출물 미리보기·Folder 더블클릭·Design Editor 열기/내보내기. **캔버스 CAD 정본화**: C-1 구성도(`/cad/arrangement`)·Design Editor 부품도(`/cad/part-drawing` — Run 도면과 동일 기하, F9 후 자동 재작도) 모두 서버 DXF 기반. **DWG 는 ODA 플러그블 준비 완료(501 안내)** — 라이선스 확정 시 env 설정만 | DWG-022~025 | ODA 대기 |
