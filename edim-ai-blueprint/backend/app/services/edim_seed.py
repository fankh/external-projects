...
UI_TRANSLATIONS_V9: dict[str, tuple[str, str, str]] = {
    ...
    "access.resetPw": ("Reset password", "パスワード再設定", "重置密码"),
    "rpt.basisIncomplete": ("Incomplete basis", "根拠不完全", "依据不完整"),
    "cpq.product": ("Product", "製品", "产品"),
    "cpq.productHint": ("Only approved products can be selected — changing it changes what is expanded and saved",
                        "承認済み製品のみ選定できます — 変更すると展開・保存の対象が変わります",
                        "仅可选择已批准的产品 — 更改后展开与保存的对象随之改变"),
    "cpq.noApproved": ("No approved product", "承認済み製品なし", "无已批准产品"),
    "cpq.noApprovedHint": ("No approved product code — approve a code first",
                           "承認済みの製品コードがありません — コード承認後に再試行してください",
                           "没有已批准的产品编码 — 请先批准编码后重试"),
    "run.quoteBasisIncomplete": ("{n} unpriced item(s) counted as 0 — the quoted amount is lower than actual",
                                 "単価未解決 {n} 件が0円で集計 — 見積金額が実際より低くなっています",
                                 "{n} 项未定价物料按0元计入 — 报价金额低于实际"),
    "run.quoteBasisIncompleteHint": ("Cost basis at issuance had unpriced items — amount is lower than actual",
                                     "発行時点の原価に単価未解決がありました — 金額が実際より低くなっています",
                                     "签发时的成本依据存在未定价物料 — 金额低于实际"),
    "run.quoteBasisUnknown": ("Issued before this check existed — basis at issuance is unknown",
                              "この点検の導入前に発行 — 発行時点の根拠は確認できません",
                              "在此校验引入前签发 — 无法确认签发时的依据"),
    "rpt.basisIncompleteHint": ("Unpriced items counted as 0 — amounts in this column are lower than actual",
                                "単価未解決の品目が0円で集計され、この列の金額は実際より低くなっています",
                                "未定价物料按0元计入，此列金额低于实际"),
    "rpt.basisIncompleteDetail": ("{n} unpriced item(s) counted as 0 — cost, revenue and EBIT are lower than actual",
                                  "単価未解決 {n} 件が0円で集計 — 原価・売上・EBIT が実際より低くなっています",
                                  "{n} 项未定价物料按0元计入 — 成本·收入·EBIT 低于实际"),
    "hier.ctxCut": ("Cut (move)", "切り取り(移動)", "剪切(移动)"),
    "hier.ctxPaste": ("Paste here", "ここに貼り付け", "粘贴到此处"),
    "hier.ctxCopy": ("Copy structure", "構造を複製", "复制结构"),
    "hier.ctxInfo": ("Properties / info", "属性・情報", "属性·信息"),
    "hier.ctxImpact": ("Impact (where used)", "影響分析(使用箇所)", "影响分析(使用处)"),
    "hier.ctxAttrs": ("Edit attributes (remark/color/lock)", "属性編集(備考·色·ロック)", "编辑属性(备注·颜色·锁定)"),
    "hier.ctxUnlock": ("Unlock", "ロック解除", "解锁"),
    "hier.ctxLock": ("Lock", "ロック", "锁定"),
    "hier.copyPrompt": ("Copy this subtree. Enter the new full address (its parent path must already exist)",
                        "この下位構造を複製します。新しい住所を完全パスで入力してください(上位パスが存在している必要があります)",
                        "复制该下级结构。请输入新的完整地址(上级路径必须已存在)"),
    "access.resetPwHint": ("Issue a temporary password — locked accounts are unlocked too. Shown only once",
                           "仮パスワードを発行 — ロック中の場合は解除も行います。表示は一度だけです",
                           "签发临时密码 — 锁定账户将一并解锁。仅显示一次"),
    ...
    "run.startNow": ("Run ▶", "実行 ▶", "执行 ▶"),
    "run.latestRun": ("Latest run", "直近の実行", "最近执行"),
    "run.idleHint": ("Opening this screen does not start a run — running creates outputs and cost data, and changes the project’s latest run",
                     "この画面を開いただけでは実行されません — 実行すると成果物・原価が生成され、プロジェクトの最新 Run が変わります",
                     "仅打开此页面不会执行 — 执行会生成产物与成本，并改变项目的最新 Run"),
...
UI_TRANSLATIONS_V28: dict[str, tuple[str, str, str]] = {
    ...
    "mrp.unmatched": ('Unmatched', '照合不可', '无法比对'),
    "mrp.unmatchedCount": ('Stock unmatched', '在庫照合不可', '库存无法比对'),
    "mrp.unmatchedHint": ('Not matched against any stock item code', '在庫コードと照合できません', '与库存编码无法比对'),
...
UI_TRANSLATIONS_V30: dict[str, tuple[str, str, str]] = {
    ...
    "printsetup.layoutHint": ('Drag on the canvas to reposition', 'キャンバスでドラッグして位置調整', '在画布上拖动调整位置'),
    "printsetup.layoutSaved": ('Placeholder layout saved', 'プレースホルダ配置を保存', '占位符布局已保存'),
    "printsetup.layoutSaveTip": ('Save placeholder layout (drag to move)', 'プレースホルダ配置を保存(ドラッグで移動)', '保存占位符布局(拖动移动)'),
    "printsetup.layoutResetTip": ('Reset to default layout (save to apply)', '既定配置に戻す(保存で反映)', '恢复默认布局(保存后生效)'),
    "printsetup.layoutDefault": ('default layout', '既定配置', '默认布局'),
    "common.unsaved": ('unsaved', '未保存', '未保存'),
    "codrel.openDwg": ('Open linked drawing', '連結図面を開く', '打开关联图纸'),
    "codrel.noDwg": ('No linked drawing — appears when a drawing with the same number is registered',
                     '連結図面なし — 同じ図番で登録すると表示', '无关联图纸 — 以相同图号登记后显示'),
    "rpt.m.revenue": ('Revenue', '売上', '销售额'),
    "rpt.m.material": ('Material cost', '材料費', '材料费'),
    "rpt.m.manufacturing": ('Manufacturing cost', '製造費', '制造费'),
    "rpt.m.direct": ('Direct expenses', '直接経費', '直接费用'),
    "rpt.m.directCostTotal": ('Direct cost total', '直接費合計', '直接成本合计'),
    "rpt.m.contributionMargin": ('Contribution margin', '貢献利益', '贡献毛益'),
    "rpt.m.sga": ('SG&A', '販管費', '销管费'),
    "rpt.m.ebit": ('EBIT', 'EBIT', 'EBIT'),
    "rpt.cmpLatest": ('Latest PCR per business type', '事業類型ごとの最新 PCR 1件', '每业务类型最新 PCR 1条'),
    "rpt.cmpNoPcr": ('No PCR — create one from the Run screen', 'PCR なし — Run 画面で作成', '无 PCR — 请在 Run 界面创建'),
    "rpt.compare": ('Compare biz. types', '事業類型比較', '业务类型比较'),
    "rpt.compareClose": ('Close comparison', '比較を閉じる', '关闭比较'),
    "rpt.compareHint": ('List PCRs by business type side by side (slide 74 form)',
                        '事業類型別 PCR を列で並べて比較 (スライド74様式)', '按业务类型并列比较 PCR (幻灯片74格式)'),
    "rpt.compareTitle": ('Business type comparison (slide 74)', '事業類型比較 (スライド74)', '业务类型比较 (幻灯片74)'),
    "rpt.metric": ('Metric', '指標', '指标'),
    "wp.dpNotFound": ('Drawing not found — could not check', '図面が見つからず点検できません', '未找到图纸，无法检查'),
    "wp.dpNoRule": ('No error rules set — enter a condition in the error-check column and save',
                    'エラー条件が未設定 — エラーチェック列に条件式を入力して保存', '未设置错误条件 — 请在错误检查列输入条件并保存'),
    "wp.dpCheck": ('Check error rules', 'エラー条件点検', '错误条件检查'),
    "wp.dpCheckTip": ('Evaluate saved error rules against current dimension values',
                      '保存されたエラー条件を現在の寸法値で評価', '用当前尺寸值评估已保存的错误条件'),
    "wp.dpViolation": ('Design error rule violated', '設計エラー条件違反', '设计错误条件违规'),
    "wp.dpNoViolation": ('No error rule violations', 'エラー条件違反なし', '无错误条件违规'),
    "wp.dpChecked": ('checked', '点検', '已检查'),
    "wp.dpUneval": ('Not evaluated', '未評価', '未评估'),
    "assist.ask": ('Ask', '質問', '提问'),
    "assist.asking": ('Querying…', '照会中…', '查询中…'),
    "assist.clear": ('New chat', '新しい会話', '新对话'),
    "assist.clearTip": ('Clear history and start over', '履歴を消して新規開始', '清除历史并重新开始'),
    "assist.fail": ('Query failed — backend required', 'クエリ失敗 — バックエンド必要', '查询失败 — 需要后端'),
    ...
    "folder.showAllRuns": ('Show all runs', '全 Run 表示', '显示全部 Run'),
    "folder.showCurrentRun": ('Current run only', '現在の Run のみ', '仅当前 Run'),
    "folder.supersededHidden": ('Past run outputs hidden', '過去 Run 成果物 非表示', '过往 Run 产出已隐藏'),
    "folder.truncated": ('Display limit reached', '表示上限に到達', '已达显示上限'),
