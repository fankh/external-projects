...
export function ProjectScreen({ active }: ScreenProps) {
  ...
  const fileCols: GridColumn<ReceivedFile>[] = [
    ...
    { key: 'type', header: '유형', width: 48, align: 'center', render: (r) => r.fileType },
    { key: 'reg', header: '등록자', width: 64, align: 'center', render: (r) => r.registrant },
    { key: 'date', header: '일자', width: 80, align: 'center', render: (r) => r.date },
    {
      key: 'act', header: '', width: 60, align: 'center',
      render: () => <span className="b" style={{ height: 18, fontSize: 10 }}>미리보기</span>,
    },
  ]
  ...
  return (
    <div className="fill-col">
      <div className="qband">
        ...
        <Chip tone="info">자동 채번 (PS-)</Chip>
        <label>영업 단계<i>*</i></label>
        <Combo width={100} value={stage} options={SALES_STAGES}
      ...
      <div style={{ display: 'flex', gap: 6, flex: 1, minHeight: 0, padding: 6 }}>
        <div className="fill-col" style={{ gap: 6, flex: 1, overflow: 'auto' }}>
          <GroupBox title={`Project — ${PROJECT.projectNo}`} right={<Chip tone="warn">Approval ☑ 대기</Chip>}>
            <div className="frm">
          ...
          <GroupBox title="영업 단계 — 변경 이력 기록">
                ...
                return (
                  <span key={s} style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}>
                    <span className={cls} style={{ cursor: 'pointer' }}
                      onClick={() => { setStage(s); setDirty(true) }}>{s}</span>
            ...
            <div style={{ fontSize: 10, color: 'var(--txt-mute)', marginTop: 4 }}>
              PS 시작 → 이후 PCR→QCR→OR 프로세스 상태기계 연결 (ERP-001/002)
            </div>
          </GroupBox>
          <GroupBox title="접수 자료 등록 (File)" style={{ flex: 1 }} noPad
            right={<Btn style={{ height: 18, fontSize: 10 }} onClick={upload}>＋ 업로드</Btn>}>
        ...
        <div style={{ width: 290, display: 'flex', flexDirection: 'column', gap: 6, overflow: 'auto' }}>
          <GroupBox title="Data Up-Load">
            <div className="frm c2">
              ...
              <div style={{ display: 'flex', gap: 4 }}>
                ...
                <Btn onClick={() => shell.setStatusMsg('중복 없음 ✓')}>중복검토</Btn>
              </div>
            </div>
          </GroupBox>
          ...
          <GroupBox title="Print 설정">
            <Btn onClick={() => shell.setStatusMsg('Print Set-up 호출 (S-3-4) — 예정 화면')}>
              🖨 Print Set-up 호출
            </Btn>
          </GroupBox>
          <GroupBox title="To-do (본인 담당 프로세스)" noPad>
            <table className="g">
              <thead><tr><th>Item</th><th>Doc.No</th><th>Task</th><th>상태</th></tr></thead>
              <tbody>
                <tr>
