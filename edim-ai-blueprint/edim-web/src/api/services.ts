...
export const ecoService = {
  ...
  async create(body: { title: string; targetType: string; targetNo: string; reason?: string; newDrawingNo?: string }): Promise<{ ecoNo: string; impact: Record<string, unknown> } | false> {
    try {
...
export const cadService = {
  ...
  async edit(fileId: number, ops: { op: 'move' | 'delete' | 'copy' | 'rotate' | 'mirror' | 'add' | 'trim'; entityId?: string; dx?: number; dy?: number; angle?: number; axis?: 'x' | 'y'; entityType?: 'line' | 'circle' | 'rect' | 'dim' | 'block'; layer?: string; x1?: number; y1?: number; x2?: number; y2?: number; radius?: number; boundaryId?: string; text?: string }[]):
