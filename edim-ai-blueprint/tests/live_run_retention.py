...
정리: 19.15 — 종전 이 자리에는 "keepLatest 를 크게 잡아 기존 Run 을 지우지 않는다" 고 적혀
있었지만 **상수 100 이 박혀 있었고**, Run 이 100건을 넘어서면서 전제가 조용히 깨져 함대를
돌릴 때마다 오래된 Run 이 영구 삭제되고 있었다(113→100 실측). 게다가 서버가 keepLatest 를
**상한 100 으로 낮추므로** 큰 값을 넘겨도 '지우지 않기' 를 만들 수 없다 — 이 스위트로는
그 경로를 만들 수 없다는 뜻이다. 그래서 ① 상한을 넘겨 요청하면 낮췄다고 말하는지(19.15
신설 필드)를 보고, ② 삭제 자체는 제품의 보관 정책(최근 100건)이 그대로 적용되는 것으로
받아들인다. 산출물 파일은 정리 대상이 아니다(#53).
