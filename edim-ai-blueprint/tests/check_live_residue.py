...
def main() -> int:
    ...
    if diffs:
        ...
        # 감소는 잔재가 늘어난 것이 아니라 **앞선 실행이 남긴 잔재를 이번에 치운** 것일 수
        # 있다. 그 경우 기준 지문이 더러운 상태에서 저장된 것이므로, DB 를 확인하고 깨끗한
        # 상태에서 한 번 더 돌리면 사라진다. 이 구분이 없어 진단을 한 번 헛돌았다(17.9).
        if any("→ 0" in d for d in diffs):
            print("  · 0 으로 줄어든 항목은 '앞선 실행의 잔재를 이번에 정리' 했을 수 있습니다 "
                  "— 기준 지문이 더러운 상태에서 저장된 경우이며, 깨끗한 상태에서 재실행하면 해소됩니다.")
