...
const cols: GridColumn<PoRow>[] = [
  ...
  { key: 'status', header: '상태', width: 90, align: 'center', sortValue: (r) => r.status, render: (r) => <Chip tone={r.status === 'DONE' ? 'ok' : r.status === 'DRAFT' ? 'info' : 'warn'}>{r.statusLabel || r.status}</Chip> },
