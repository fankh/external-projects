...
export default async function TenantMenusPage() {
  ...
  const [left, head, myLeft, myHead] = await Promise.all([getTenantNav(), getTenantHeadNav(), getLeftNav(), getHeadNav()])
  return (
    <div className="fill-col" style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, minHeight: 0, padding: 6 }}>
        <TenantMenuAdmin initialLeft={left} initialHead={head} myLeft={myLeft} myHead={myHead} />
      </div>
    </div>
  )
}
