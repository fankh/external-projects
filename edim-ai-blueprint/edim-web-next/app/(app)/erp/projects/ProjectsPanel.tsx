...
export function ProjectRegForm() {
  ...
  return (
    <form action={action} style={{ display: 'flex', gap: 4, alignItems: 'center', flexWrap: 'wrap' }}>
      <input className="in req" name="projectName" placeholder="프로젝트명" style={{ width: 160 }} />
      ...
      <input className="in" name="item" placeholder="Item (AHU 등)" style={{ width: 100 }} />
      <input className="in req" name="client" placeholder="고객사" style={{ width: 110 }} />
      <input className="in" name="clientContact" placeholder="고객 담당" style={{ width: 90 }} />
      <button className="b run" type="submit" disabled={pending}>＋ 프로젝트 등록</button>
...
export function ProjectStagePanel({ no, stage, updatedAt }: { no: string; stage: string; updatedAt: string }) {
  ...
  return (
    <div className="gb" style={{ padding: 8, fontSize: 11, display: 'flex', flexDirection: 'column', gap: 6 }}>
      <div style={{ fontWeight: 700, color: 'var(--title-navy)' }}>영업 단계 — {no}</div>
