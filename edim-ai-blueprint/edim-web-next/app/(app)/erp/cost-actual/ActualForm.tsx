...
export function ActualForm() {
  ...
  return (
    <form ref={ref} action={action} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '4px 8px', borderBottom: '1px solid var(--line)', flexWrap: 'wrap' }}>
      ...
      <select name="category" className="in" defaultValue="MATERIAL" style={inp}>
      ...
      <input name="itemName" className="in" placeholder="예: Casing" style={{ ...inp, width: 120 }} />
      <label style={{ fontSize: 11 }}>PO</label>
      <input name="poNo" className="in" placeholder="선택" style={{ ...inp, width: 80 }} />
      <label style={{ fontSize: 11 }}>수량</label>
      <input name="qty" className="in" defaultValue="1" style={{ ...inp, width: 48 }} required />
      <label style={{ fontSize: 11 }}>단가</label>
      <input name="unitPrice" className="in" defaultValue="0" style={{ ...inp, width: 80 }} required />
      <label style={{ fontSize: 11 }}>프로젝트</label>
      <input name="projectNo" className="in" placeholder="PS-… 선택" style={{ ...inp, width: 84 }} />
      <button type="submit" className="b pri" disabled={pending} style={inp}>{pending ? '적재 중…' : '실적 적재'}</button>
