...
export function InboundForm() {
  ...
  return (
    <form ref={ref} action={action} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '4px 8px', borderBottom: '1px solid var(--line)', flexWrap: 'wrap' }}>
      ...
      <input name="itemCode" className="in" placeholder="품목코드" style={{ ...inp, width: 90 }} required />
      <label style={{ fontSize: 11 }}>위치</label>
      <input name="locationCode" className="in" defaultValue="GEN-A01" style={{ ...inp, width: 90 }} required />
      <label style={{ fontSize: 11 }}>수량</label>
      <input name="quantity" className="in" defaultValue="1" style={{ ...inp, width: 56 }} required />
      <label style={{ fontSize: 11 }}>Lot</label>
      <input name="lotNo" className="in" placeholder="선택" style={{ ...inp, width: 70 }} />
      <label style={{ fontSize: 11 }}>S/N</label>
      <input name="serialNo" className="in" placeholder="선택" style={{ ...inp, width: 70 }} />
      <label style={{ fontSize: 11 }}>단가</label>
      <input name="unitPrice" className="in" placeholder="STOCK 자동" style={{ ...inp, width: 78 }} />
      <button type="submit" className="b pri" disabled={pending} style={inp}>{pending ? '입고 중…' : '입고'}</button>
