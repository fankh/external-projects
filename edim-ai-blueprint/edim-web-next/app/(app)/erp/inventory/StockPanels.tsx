...
export function StockPanels({ atp, reservations, movements }: {
  ...
  return (
    <div style={{ display: 'flex', gap: 6, minHeight: 0, flex: 1 }}>
      <GroupBox title={`가용재고 ATP — ${atp.length}종`} noPad style={{ flex: 1, minHeight: 0, overflow: 'auto' }}>
        <table className="g" style={{ width: '100%' }}>
          <thead><tr><th>품목</th><th>보유</th><th>예약</th><th>가용</th></tr></thead>
          <tbody>{atp.map((r) => (
      ...
      <GroupBox title={`재고 예약 — ${reservations.length}건 (ACTIVE)`} noPad style={{ flex: 1.2, minHeight: 0, display: 'flex', flexDirection: 'column' }}>
        <div style={{ flex: 1, minHeight: 0, overflow: 'auto' }}>
          <table className="g" style={{ width: '100%' }}>
            <thead><tr><th>#</th><th>품목</th><th>수량</th><th>참조</th><th></th></tr></thead>
            <tbody>{reservations.length ? reservations.map((r) => (
              <tr key={r.reservationId}>
                ...
                <td className="c"><button className="b" disabled={pending} style={{ height: 17, fontSize: 10 }}
                  onClick={() => start(async () => setSt(await releaseReservation(r.reservationId)))}>해제</button></td>
              </tr>
            )) : <tr><td colSpan={5} style={{ textAlign: 'center', color: 'var(--txt-mute)' }}>예약 없음</td></tr>}</tbody>
          </table>
        </div>
        <div style={{ display: 'flex', gap: 4, padding: 4, borderTop: '1px solid var(--line)', alignItems: 'center', flexWrap: 'wrap' }}>
          <input className="in" style={{ width: 90 }} placeholder="품목 코드" value={item} onChange={(e) => setItem(e.target.value)} />
          <input className="in" style={{ width: 48, textAlign: 'right' }} placeholder="수량" value={qty} onChange={(e) => setQty(e.target.value)} />
          <input className="in" style={{ width: 84 }} placeholder="참조 (WO 등)" value={ref} onChange={(e) => setRef(e.target.value)} />
      ...
      <GroupBox title={`입출고 이력 (Lot 추적) — 최근 ${movements.length}건`} noPad style={{ flex: 1.2, minHeight: 0, overflow: 'auto' }}>
        <table className="g" style={{ width: '100%' }}>
          <thead><tr><th>품목</th><th>구분</th><th>수량</th><th>Lot</th><th>참조</th><th>일시</th></tr></thead>
          ...
          )) : <tr><td colSpan={6} style={{ textAlign: 'center', color: 'var(--txt-mute)' }}>이력 없음</td></tr>}</tbody>
        </table>
      </GroupBox>
    </div>
  )
