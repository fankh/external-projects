    ...
    <div className="fill-col" style={{ height: '100%', display: 'flex', gap: 6 }}>
      <div style={{ flex: 1.3, minWidth: 0 }}>
        <DenseGrid prefKey="next-macros" colFilter columns={cols} rows={rows}
          ...
          onRowClick={select} emptyText="Macro 가 없습니다" />
      </div>
      <div className="gb" style={{ width: 380, display: 'flex', flexDirection: 'column', gap: 6, padding: 8, fontSize: 11, overflow: 'auto' }}>
        <div style={{ fontWeight: 700, color: 'var(--title-navy)' }}>Macro 편집 {selName ? `— ${selName}` : '(행 클릭 또는 신규 이름 입력)'}</div>
        <input className="in req" placeholder="Macro 이름 (TBX-…)" value={name} onChange={(e) => setName(e.target.value)} />
        <textarea className="in" placeholder="=IF(A>0, A*B, 0) — Excel 호환 식" value={expr} onChange={(e) => setExpr(e.target.value)}
          style={{ fontFamily: 'Consolas, monospace', fontSize: 11, height: 70, resize: 'vertical' }} />
        <input className="in" placeholder="Prompt (자연어 설명)" value={prompt} onChange={(e) => setPrompt(e.target.value)} />
        <div style={{ display: 'flex', gap: 4, alignItems: 'center', flexWrap: 'wrap' }}>
          <input className="in" style={{ flex: 1, minWidth: 110, fontFamily: 'Consolas, monospace' }} placeholder="Test 변수: A=560, B=2"
        ...
        {evalR ? (
          <div style={{ border: '1px solid var(--line)', padding: 6, background: '#FAFBFC' }}>
            {evalR.ok
              ? <div>결과 = <b style={{ color: 'var(--run)' }}>{String(evalR.value)}</b></div>
              : <div style={{ color: 'var(--err)' }}>오류 — {evalR.error}</div>}
            {evalR.trace?.length ? (
        ...
        <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
          <button className="b run" disabled={pending} onClick={() => start(async () => setSt(await saveMacro(name, expr, prompt)))}>저장 (F12)</button>
          <button className="b" disabled={pending || !selName} onClick={() => selName && start(async () => setSt(await requestMacroApproval(selName)))}>승인 요청</button>
