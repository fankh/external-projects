  ...
  return (
    <div className="fill-col" style={{ height: '100%', display: 'flex', flexDirection: 'column', gap: 4 }}>
      <form action={regAction} style={{ display: 'flex', gap: 4, alignItems: 'center', flexWrap: 'wrap' }}>
        <input className="in req" name="code" placeholder="재질 코드" style={{ width: 90 }} />
        <input className="in req" name="name" placeholder="재질명" style={{ width: 120 }} />
        <select className="in" name="materialType" defaultValue="STEEL" style={{ width: 86 }}>
          {['STEEL', 'STAINLESS', 'AL', 'PLASTIC', 'ETC'].map((t) => <option key={t}>{t}</option>)}
        </select>
        <input className="in" name="density" placeholder="밀도" style={{ width: 62 }} />
        <input className="in" name="standard" placeholder="규격 (KS 등)" style={{ width: 96 }} />
        <input className="in" name="hazard" placeholder="위험 표기" style={{ width: 76 }} />
        <button className="b run" type="submit" disabled={regPending}>＋ 재질 등록</button>
        <span className="sep" />
        <input className="in" style={{ width: 100 }} placeholder="새 재질명 (수정)" value={newName} onChange={(e) => setNewName(e.target.value)} />
        <input className="in" style={{ width: 62 }} placeholder="새 밀도" value={newDensity} onChange={(e) => setNewDensity(e.target.value)} />
      ...
      <div style={{ flex: 1, minHeight: 0 }}>
        <DenseGrid prefKey="next-materials" colFilter columns={cols} rows={rows}
          ...
          onRowClick={(r) => setSelCode(r.code)} emptyText="재질이 없습니다" />
      </div>
    </div>
  )
}
