...
import { SlotGrid, type GroupRow, type SlotRow } from './SlotGrid'
...
export default async function SubCodePage({ searchParams }: { searchParams: Promise<{ group?: string }> }) {
  ...
  try {
    ...
    groups = await apiServer<GroupRow[]>('/codes/groups')
  } catch (e) {
  ...
  return (
    <div className="fill-col" style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, minHeight: 0, padding: 6, display: 'flex', gap: 6 }}>
        {err ? <div style={{ padding: 12, fontSize: 11, color: 'var(--err)' }}>백엔드 오류 — {err}</div> : <div style={{ flex: 1, minWidth: 0 }}><SlotGrid rows={rows} group={group} groups={groups} /></div>}
        <SubWorkPlace />
      </div>
    </div>
  )
