...
export function I18nEditor({ rows, entity, locale }: { rows: DataTransRow[]; entity: string; locale: string }) {
  ...
  const cols: GridColumn<DataTransRow>[] = [
    ...
    { key: 'source', header: '원문', render: (r) => r.source },
    { key: 'value', header: `번역 (${locale})`, editable: true, render: (r) => r.value ? r.value : <span style={{ color: 'var(--txt-mute)' }}>—</span> },
    { key: 'status', header: '상태', width: 72, align: 'center', render: (r) => r.value ? <Chip tone="ok">번역됨</Chip> : <Chip tone="warn">미번역</Chip> },
  ]
  ...
  return (
    <div className="fill-col" style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      <div style={{ display: 'flex', gap: 6, alignItems: 'center', padding: '4px 6px' }}>
        ...
        <label style={{ fontSize: 11 }}>언어</label>
        <select className="in" value={locale} onChange={(e) => go(entity, e.target.value)} style={{ height: 22, fontSize: 11 }}>
          {LOCALES.map((l) => <option key={l} value={l}>{l}</option>)}
        </select>
        <span style={{ fontSize: 10, color: 'var(--txt-mute)' }}>번역 셀 더블클릭 → 입력 → Enter (빈 값=삭제)</span>
        {msg ? <span style={{ marginLeft: 'auto', fontSize: 11, color: 'var(--run)' }}>{pending ? '저장 중…' : msg}</span> : null}
      </div>
      <div style={{ flex: 1, minHeight: 0 }}>
        <DenseGrid prefKey={`next-i18n-${entity}`} colFilter columns={cols} rows={rows}
          rowKey={(r) => r.entityId} onCellEdit={(row, _i, _k, value) => commit(row, value)} emptyText="항목이 없습니다" />
      </div>
    </div>
  )
}
