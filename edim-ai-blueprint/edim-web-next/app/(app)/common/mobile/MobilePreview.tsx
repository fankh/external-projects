...
const TABS = [{ k: 'approval', label: '승인함' }, { k: 'task', label: '업무함' }, { k: 'inbound', label: '입고' }] as const
    ...
    <div style={{ display: 'flex', justifyContent: 'center', padding: 16, height: '100%', overflow: 'auto' }}>
      ...
      <div style={{ width: 360, minHeight: 640, border: '10px solid #1B2233', borderRadius: 32, background: '#fff', display: 'flex', flexDirection: 'column', overflow: 'hidden', boxShadow: '0 8px 30px rgba(20,26,40,.25)' }}>
        ...
        <div style={{ display: 'flex', borderBottom: '1px solid var(--line)' }}>
          {TABS.map((t) => (
            <button key={t.k} onClick={() => setTab(t.k)} style={{ flex: 1, padding: '8px 0', fontSize: 12, border: 'none', background: tab === t.k ? '#EDF2FA' : '#fff', color: tab === t.k ? 'var(--title-navy)' : 'var(--txt-mute)', fontWeight: tab === t.k ? 700 : 400, cursor: 'pointer' }}>
              {t.label}{t.k === 'approval' && inbox.length ? ` (${inbox.length})` : ''}{t.k === 'task' && events.length ? ` (${events.length})` : ''}
            </button>
          ))}
        </div>
          ...
          {tab === 'approval' ? (
            inbox.length ? inbox.map((r) => (
              <div key={r.id} style={{ border: '1px solid var(--line)', borderRadius: 8, padding: 10, marginBottom: 8 }}>
                ...
                <div style={{ display: 'flex', gap: 6, marginTop: 6 }}>
                  <button className="b" disabled={pending} onClick={() => decide(r.id, true)} style={{ flex: 1, height: 30, fontSize: 12, color: 'var(--run)' }}>승인</button>
                  <button className="b" disabled={pending} onClick={() => decide(r.id, false)} style={{ flex: 1, height: 30, fontSize: 12, color: 'var(--err)' }}>반려</button>
                </div>
              </div>
            )) : <Empty text="대기 중인 승인 요청 없음" />
          ) : tab === 'task' ? (
            events.length ? events.map((e) => (
              <div key={e.eventId} style={{ border: '1px solid var(--line)', borderRadius: 8, padding: 10, marginBottom: 8 }}>
                ...
                <div style={{ fontSize: 11, color: 'var(--txt-mute)', margin: '3px 0' }}>{e.procName} · {e.owner} · {e.deadline}{e.delayed ? ' (지연)' : ''} <Chip tone={e.status === '지연' ? 'warn' : 'info'}>{e.status}</Chip></div>
                <button className="b" disabled={pending || e.status === 'DONE'} onClick={() => complete(e.eventId)} style={{ width: '100%', height: 30, fontSize: 12, marginTop: 4 }}>완료 처리</button>
              </div>
            )) : <Empty text="업무 없음" />
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              <label style={{ fontSize: 12 }}>품목 코드</label>
              <input className="in" value={item} onChange={(e) => setItem(e.target.value)} style={{ height: 32, fontSize: 13 }} aria-label="품목 코드" />
              <label style={{ fontSize: 12 }}>수량</label>
              <input className="in" value={qty} onChange={(e) => setQty(e.target.value)} style={{ height: 32, fontSize: 13 }} aria-label="수량" />
              <button className="b" disabled={pending} onClick={doInbound} style={{ height: 36, fontSize: 13, marginTop: 4 }}>입고 처리 (MI-002)</button>
            </div>
          )}
        </div>
