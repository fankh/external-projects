...
export default async function CodeDetailPage({ searchParams }: { searchParams: Promise<{ code?: string; name?: string }> }) {
  ...
  return (
    <div className="fill-col" style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      <ScreenHeader title={`코드 상세 — ${code}${name ? ` (${name})` : ''}`} source="/prices · /codes/{c}/referencers · slot-items · approval-history" />
      {err ? <div style={{ padding: 12, fontSize: 11, color: 'var(--err)' }}>백엔드 오류 — {err}</div> : (
        <div style={{ display: 'flex', gap: 6, flex: 1, minHeight: 0, padding: 6 }}>
          <div className="fill-col" style={{ gap: 6, flex: 1.2, overflow: 'auto' }}>
            <div className="gb">
              <div style={{ fontSize: 11, fontWeight: 600, padding: '3px 6px' }}>대표 도면 (Block)</div>
              <CodeBlockPreview code={code} name={name} />
            </div>
            <div className="gb">
              <div style={{ fontSize: 11, fontWeight: 600, padding: '3px 6px' }}>단가 이력 — {prices.length}건 (cst_price)</div>
              {prices.length ? (
                <table className="g"><thead><tr><th>Supplier</th><th>Price</th><th>소스</th><th>적용 시작</th><th>종료</th><th>상태</th></tr></thead>
                  <tbody>{prices.map((p, i) => (
                    <tr key={i}><td className="c">{p.supplier}</td><td className="num">{p.price.toLocaleString()}</td>
                      ...
                      <td className="c">{p.active ? <Chip tone="ok">적용중</Chip> : <Chip tone="warn">만료</Chip>}</td></tr>
                  ))}</tbody></table>
              ) : <div style={{ padding: 10, fontSize: 11, color: 'var(--txt-mute)' }}>등록 단가 없음 — Pricing Run 시 warn 대상</div>}
            </div>
            <div className="gb">
              <div style={{ fontSize: 11, fontWeight: 600, padding: '3px 6px' }}>Referencers (Where-Used) — {referencers.length}건</div>
              {referencers.length ? (
                <table className="g"><thead><tr><th>Mother</th><th>Desc.</th><th>Q'ty</th><th>구분</th></tr></thead>
              ...
              ) : <div style={{ padding: 10, fontSize: 11, color: 'var(--txt-mute)' }}>상위 참조 없음 — 최상위 또는 미연결 코드</div>}
            </div>
          </div>
          <div className="side-scroll" style={{ width: 320, display: 'flex', flexDirection: 'column', gap: 6, overflow: 'auto' }}>
            <div className="gb" style={{ padding: 8 }}>
              <div style={{ fontSize: 11, fontWeight: 600, marginBottom: 4 }}>코드 정보</div>
              <div style={{ fontSize: 11, lineHeight: 1.9 }}>
                <div>유형 <b>{code.startsWith('KDCR') ? 'PRODUCT' : 'SUB'}</b></div>
                <div style={{ fontFamily: 'Consolas, monospace' }}>Hierarchy /C/ENG/FAN/{base}</div>
              </div>
            </div>
            {slots.length ? (
              <div className="gb">
                <div style={{ fontSize: 11, fontWeight: 600, padding: '3px 6px' }}>필수 슬롯 정의 ({slots.length})</div>
                <table className="g"><thead><tr><th>Slot</th><th>항목</th><th>필수</th></tr></thead>
                  <tbody>{slots.map((s) => (
                    <tr key={s.pcItemId}><td className="c"><b>{s.slot}</b></td><td>{s.itemName}</td>
                      <td className="c">{s.required ? <Chip tone="err">필수</Chip> : <Chip tone="info">선택</Chip>}</td></tr>
                  ))}</tbody></table>
              </div>
            ) : null}
            <div className="gb">
              <div style={{ fontSize: 11, fontWeight: 600, padding: '3px 6px', display: 'flex' }}>승인 이력<span style={{ marginLeft: 'auto', fontSize: 9.5, color: 'var(--txt-mute)' }}>{hist.length}건</span></div>
              {hist.length ? (
                <table className="g"><thead><tr><th>일자</th><th>행위</th><th>처리자</th></tr></thead>
                  <tbody>{hist.map((h, i) => <tr key={i} title={h.note}><td className="c">{h.date}</td><td>{h.action}</td><td className="c">{h.by}</td></tr>)}</tbody></table>
              ) : <div style={{ padding: 10, fontSize: 11, color: 'var(--txt-mute)' }}>승인 이력 없음</div>}
            </div>
          </div>
        </div>
      )}
