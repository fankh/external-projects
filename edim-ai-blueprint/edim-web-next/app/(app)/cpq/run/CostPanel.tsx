  ...
  return (
    <div style={{ display: 'flex', gap: 6, flex: 'none', maxHeight: 200, padding: '0 6px 6px' }}>
      <div className="gb" style={{ flex: 1, overflow: 'auto', display: 'flex', flexDirection: 'column' }}>
        <div style={{ fontSize: 11, fontWeight: 600, padding: '3px 6px', display: 'flex' }}>원가 상세 (cst_calc)<span style={{ marginLeft: 'auto' }}>{costs?.length ? <Chip tone="ok">3분류</Chip> : <Chip tone="warn">미적재</Chip>}</span></div>
        {costs && costs.length ? (
          <table className="g"><thead><tr><th>분류</th><th>내역</th><th style={{ textAlign: 'right' }}>합계</th></tr></thead>
            <tbody>
              ...
              <tr><td className="c" colSpan={2}><b>직접비 계</b></td><td className="num"><b>{costs.reduce((s, c) => s + c.total, 0).toLocaleString()}</b></td></tr>
            </tbody></table>
        ) : <div style={{ padding: 8, fontSize: 11, color: 'var(--txt-mute)' }}>원가 상세 없음 — 이 Run 실행분부터 cst_calc 적재</div>}
      </div>
      <div className="gb" style={{ flex: 1.3, overflow: 'auto', display: 'flex', flexDirection: 'column' }}>
        <div style={{ fontSize: 11, fontWeight: 600, padding: '3px 6px' }}>PCR 수익성 → 견적 (cst_pcr·cst_quotation)</div>
        <div style={{ display: 'flex', gap: 4, padding: 6, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <button className="b" disabled={pending || !costs?.length} onClick={makePcr} style={{ height: 22, fontSize: 11 }}>PCR 생성</button>
          ...
          <button className="b" disabled={pending} onClick={makeQuote} style={{ height: 22, fontSize: 11 }}>견적 확정</button>
          {pcr ? <span style={{ fontSize: 10.5 }}>기여마진 <b>{(pcr.contributionMargin / 1000).toLocaleString()}K</b> · EBIT <b style={{ color: pcr.ebit >= 0 ? 'var(--ok)' : 'var(--err)' }}>{(pcr.ebit / 1000).toLocaleString()}K</b></span> : null}
        </div>
        {quotes.length ? (
          <table className="g"><thead><tr><th>No.</th><th>금액</th><th>세액</th><th>일자</th><th>PDF</th></tr></thead>
            <tbody>{quotes.slice(0, 4).map((q) => (
              <tr key={q.quotationId}>
                ...
                <td className="c"><a className="b" href={`/api/cost/quotation-pdf?id=${q.quotationId}`} target="_blank" rel="noreferrer" style={{ height: 18, fontSize: 10 }}>미리보기</a></td>
              </tr>
            ))}</tbody></table>
        ) : <div style={{ padding: '0 8px 6px', fontSize: 10.5, color: 'var(--txt-mute)' }}>확정 견적 없음 — PCR 생성 후 견적 확정</div>}
