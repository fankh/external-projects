    ...
    <div className="fill-col" style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      <div style={{ display: 'flex', gap: 6, flex: 1, minHeight: 0, padding: 6 }}>
        <div className="fill-col" style={{ gap: 6, flex: 1, overflow: 'auto' }}>
          <GroupBox title={tr('doctpl.inputOutput', 'Input Data → Output Data (Macro 계산)')}>
            <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap', padding: 4 }}>
              ...
              <span className="unit">%</span>
              <Btn variant="run" onClick={calc}>{tr('doctpl.macroCalcF9', '▶ Macro 계산 F9')}</Btn>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6, flex: 1, minHeight: 0 }}>
            <GroupBox title={tr('doctpl.psychroChart', '습공기 선도 — 계산점 표시')}>
            ...
            <GroupBox title={tr('doctpl.spreadsheet', '스프레드시트 (A~L — 중간 계산)')} noPad>
              <table className="g">
                ...
                <tbody>
                  ...
                  <tr><td className="c"><b>2</b></td><td>P(kPa)</td><td className="num">101.325</td><td>{tr('doctpl.correction', '보정')}</td><td className="num">{(1 - 0.0035 * (Number(humid) - 50) / 50).toFixed(4)}</td></tr>
