    ...
    return [
      ...
      },
      {
        ...
        bullets: [
          ...
          `외부·엔드포인트 위협 미조치 — 크리덴셜 노출 ${credOpen.length} · 휴면 계정 ${acctOpen.length} · 미인가 SW ${swOpen.length} · USB 매체 ${usbOpen.length} · 로컬 VM ${vmOpen.length} · 다크웹 유출 ${s.leaks.filter((l) => l.status !== '조치 완료').length}`,
