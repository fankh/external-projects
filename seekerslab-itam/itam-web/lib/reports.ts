...
export function buildSections(kind: ReportKind): ReportSection[] {
    ...
    return [
      ...
      },
      {
        ...
        note: degraded.length
          ? `분실·폐기·수리·반납대기 등 저하·이탈 상태이면서 하위 의존이 있는 상위 ${degraded.length}건 — 장애가 하위로 전이됩니다`
  ...
  const dqSection: ReportSection = flagged.length === 0
    ...
    : {
        ...
        rows: flagged.map((a) => [a.assetNo, a.category, a.status, assetDataIssues(a, deptOfOwner(s.users)).join(', ')]),
      }
