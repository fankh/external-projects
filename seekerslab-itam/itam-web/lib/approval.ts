...
export function canDecideApproval(role: Role, a: Approval, viewer?: string): boolean {
  ...
  //  직무 분리 — 본인이 폼으로 올린 상신(selfSubmitted)은 본인이 결재할 수 없다. 서버 decide 와 결재함
  //   버튼이 쓰는 규칙 그대로다. 그전에는 이 판정에 규칙이 없어, 부르는 쪽들이 각자 'requester !== 나' 를
  //   덧붙였다 — 더 넓은 규칙이라 시스템·운영자 상신(selfSubmitted 없음)까지 함께 빠졌다. 그래서 결재함은
  //   승인 버튼을 내주고 서버도 허용하는 건이 대시보드·상단 배지·어시스턴트에서는 사라졌다
  //   (실제로 보안담당의 본인 상신 격리 요청 — 필수·SLA 초과 — 이 배지에서 빠져 있었다).
  if (viewer && a.selfSubmitted && a.requester === viewer) return false
