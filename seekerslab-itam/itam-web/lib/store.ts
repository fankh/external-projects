...
function seedExternal(): ExternalAsset[] {
  return [
    //  인증서 유효기간은 '아직 유효한가'를 시계와 견주는 값이다 — 아직 오지 않은 날을 고정으로 적으면
    //   그 날이 지나는 순간 유효 픽스처가 만료로 뒤집힌다. CT 채널은 유효 인증서로 생존을 추정하므로,
    //   유효·만료 짝(ct-active · ct-legacy)이 무너지면 '생존 유력 추정'을 확인하는 검사가 근거를 잃는다.
    //   지난 유효기간은 계속 만료라 그대로 둔다 — 막아야 하는 것은 미래 고정 날짜뿐이다.
    //   시드 기준일 오프셋을 유지해 기준일 렌더는 그대로다(회차 기한과 같은 규약).
...
function seed(): Store {
  return {
    ...
    contracts: [
      ...
      //  이 두 구매 계약의 만료일은 달력 사실이 아니라 '만료가 다가오는데 발주가 저조하다'는 상태다 —
      //   발주 미이행 위험의 유일한 픽스처이고, 기간이 끝난 계약은 위험·독촉에서 빠지므로(로 #769)
      //   고정 날짜로 두면 그 날이 지나는 순간 이 판정을 검증할 대상이 사라진다. 기준일 2026-07-29 기준
      //   오프셋 계열(D+33 · D+40)로 둔다 — e2e 가 중간에 운영 정책 만료창을 60일로 좁히므로, 좁힌
      //   창 안에 들어야 그 뒤 판정도 계속 임박으로 남는다(원래 D+78 은 좁힌 창 밖이었다).
      ...
      //  집행이 한 건도 없는 유지보수 계약 픽스처 — 기준일에는 기간이 남아 있었다(D+22). 고정 만료일이라
      //   실제 시간이 흐르며 기간이 끝나 버렸고, 그러면 이 계약은 집행할 수 없는데도 이행 독촉 대상으로
      //   남는다. 기준일 오프셋을 유지한 상대값으로 두어 설계된 상태(기간 중 미집행)를 지킨다.
      { id: 'CT-2024-011', kind: '유지보수', name: '스토리지·백업 유지보수', vendor: '효성인포', start: '2025-09-01', end: addDays(today(), 22), amount: 36_000_000, ownerDept: '인프라운영팀' },
      // 발주 미이행 임계 회귀 — 발주율 79.6%(반올림 80%) · 만료 임박(dday<90). 반올림 기준이면 위험 큐에서 빠지지만 실집행 기준이면 미이행 위험이어야 한다(IN-2608-55 로트가 79.6M 발주).
      { id: 'CT-2026-055', kind: '구매', name: '보안장비 도입(회귀)', vendor: '(주)넷시큐어', start: '2026-06-01', end: addDays(today(), 40), amount: 100_000_000, ownerDept: '보안운영팀' },
...
export const SCHEMA_SHAPE = '11824b32'
