...
export interface Dispatch {
  ...
  to: string
  subject: string
  kind: '소유자 확인' | '만료 임박' | '격리 통보' | '에스컬레이션' | '리포트 배포' | '위협 대응' | '입고 반려' | '계약 해지' | '공지 독촉' | '결재 결과' | '결재 독촉' | '대여 독촉' | '수리 독촉' | '정기 점검 독촉' | '입고 독촉' | '발주 요청' | '장애 신고' | '수리 결과' | '수령 확인' | 'QnA 답변' | 'QnA 접수' | 'QnA 독촉' | '자산 불출' | '자산 대여' | '자산 이동' | '반납 접수' | 'MFA 등록 요구' | '유지보수 예산 통보' | 'SaaS 판정 독촉'
  ref?: string
}
