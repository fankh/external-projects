...
/** 자산 상태 전량(수명주기 순) — ASSET_CATEGORIES 와 같은 규약으로 타입의 런타임 짝을 둔다.
 *  이 목록이 없어 대장 필터와 어시스턴트의 상태별 분포가 각자 아홉 개를 손으로 적고 있었다(순서도 서로 달랐다).
 *  상태가 하나 늘면 어느 화면에선 필터에 안 뜨고, 어시스턴트는 '총 보유 N대'라고 말하면서 분포 합은 N 이 안
 *  되는 조용한 잘림이 난다 — 부분의 합이 전체와 다른 답을 사람이 알아채기는 어렵다. 한 곳에서 읽는다. */
export const ASSET_STATUSES: AssetStatus[] = ['검수중', '사용중', '유휴', '대여중', '반납대기', '수리중', '분실', '폐기예정', '폐기완료']
...
/** 변경 이력 이벤트의 강조 분류 — 수명주기 중대 이벤트를 타임라인에서 눈에 띄게 하는 기준(제품안내서 §03 변경 이력).
 *  위험=폐기·분실 / 정비=점검·수리 / 진입=등록·편입. 여기 없는 종류(불출·이동·반납·대여·구성변경·보증연장)는 기본 표기다.
 *
 *  **분류만 공유하고 표현은 각 산출물이 정한다** — 화면 타임라인은 Chip 톤(err·warn·ok), 인쇄 카드는 인쇄용 hex 를 쓴다.
 *  그전에는 두 곳이 여섯 종류를 각자 나열했고, 인쇄 카드 주석은 '화면 변경 이력 타임라인과 동일 언어'라고 적어 두었다 —
 *  종류를 하나 더하거나 옮길 때 한쪽만 고치면 그 주석이 거짓이 되고, 같은 자산의 같은 이력이 화면에서는 강조되는데
 *  인쇄물에서는 평범하게 나간다(인수인계·감사에 종이로 나가는 산출물이다). */
export const EVENT_EMPHASIS: Record<string, '위험' | '정비' | '진입'> = {
  폐기: '위험', 분실: '위험', 점검: '정비', 수리: '정비', 등록: '진입', 편입: '진입',
}
...
export interface Asset {
  ...
  /** NAC 격리 집행일 — AI 이상행위 탐지 격리 요청이 결재 승인돼 네트워크 차단이 집행된 시점(대장 관리 자산).
   *  발견 자산 격리(discovered.action='격리완료')의 대장 자산 대응 — 상태는 유지한 채 네트워크만 격리된다. */
...
export interface AssetRepairCost {
  ...
  /** 업체 SLA — 예상 반환일(eta) 이내 반환 여부. 완료 시점에 판정(true=정시·false=지연). eta 없이 의뢰된 건은 미설정(정시율 분모 제외). */
  onTime?: boolean
}
...
/** 재배치 대기 풀 — 보유자를 떠나 다시 배정할 수 있는(또는 곧 그렇게 될) 상태. 재고 집계의 '유휴·반납대기' 칸,
 *  리포트의 유휴 서술, 실사 위치 판정이 모두 이 한 묶음을 쓴다. 각자 배열을 적어 두면 상태가 하나 늘 때
...
export interface Dispatch {
  ...
  to: string
  subject: string
  kind: '소유자 확인' | '만료 임박' | '격리 통보' | '에스컬레이션' | '리포트 배포' | '위협 대응' | '입고 반려' | '계약 해지' | '공지 독촉' | '결재 결과' | '결재 독촉' | '대여 독촉' | '수리 독촉' | '정기 점검 독촉' | '입고 독촉' | '발주 요청' | '장애 신고' | '수리 결과' | '수령 확인' | 'QnA 답변' | 'QnA 접수' | 'QnA 독촉' | 'QnA 해결' | '자산 불출' | '자산 대여' | '자산 이동' | '반납 접수' | '분실 신고' | 'MFA 등록 요구' | '유지보수 예산 통보' | 'SaaS 판정 독촉' | '발주 이행 독촉' | '재물조사 독촉' | 'EOL 업그레이드 통보' | '교체 검토 통보' | '라이선스 제거 요청' | '유지보수 이행 독촉' | '라이선스 재계약 검토' | '부속서류 제출 요청' | '유지보수 SLA 위반 독촉' | '발주 정산 종결' | '의존 영향 통지'
  ref?: string
  /** 전달 상태 — 배치·연동 서버의 발송 결과. 미지정은 발송 성공(기본). '실패'는 반송·게이트웨이 오류 등으로 재발송 대상(§06 이메일·문자 발송 신뢰성). */
  deliveryStatus?: '발송' | '실패'
  /** 재발송 증적 — 실패했던 최초 발송 시각과 재발송 처리자. 재발송은 at 을 최신 시도 시각으로 덮는데
   *  (당일 중복 억제가 at 을 본다) 그러면 '언제 처음 보냈다가 실패했는지'가 행에서 사라져, 발송 이력이
   *  그 건을 처음부터 정상 발송된 것처럼 말한다. 통지 증적은 '보냈다'가 아니라 '언제·몇 번 보냈다'까지가
   *  사실이다(전역 감사로그에만 남기면 행만 보는 사람은 알 수 없다). */
  failedAt?: string
  resentBy?: string
}
...
export interface Contract {
  ...
  /** 해지 확정일 */
  terminatedAt?: string
  /** 해지 사유 — 해지는 사유를 필수로 받는데 그동안 감사로그와 통지 제목에만 남았다.
   *  해지된 계약·라이선스를 나중에 보는 사람(재계약 검토·공급사 평가·중복 구독 정리)에게는
   *  '왜 끊었는가'가 곧 판단 근거다 — 그 근거가 대장이 아니라 전역 로그에만 있으면 찾을 수 없다. */
  terminateReason?: string
  /** 해지 처리자 — 결재(decidedBy)·폐기 소거(wipedBy)·입고 검수(inspector)와 같은 규약.
   *  해지는 되돌릴 수 없는 판정이고 만료 임박 집계·알림·컴플라이언스에서 그 건을 빼는 결과를 낳는다.
   *  언제·왜에 이어 누가까지 있어야 증적이 닫힌다 — 감사에서 '누가 끊었나'는 전역 로그가 아니라
   *  그 계약에서 답해야 하는 질문이다. */
  terminatedBy?: string
  /** 발주 정산 종결일 — 발주가 전량 입고·검수 완료된 구매 계약을 정산 완료(대금 정산 근거 확정)로 닫은 시점.
   *  종결분은 발주 이행 현황·미이행 위험 집계에서 빠진다(생애주기 종착점). 제품안내서 §03 구매 계약: 검수 완료액=대금 정산 근거. */
  settledAt?: string
  ...
  sla?: string
  /** SLA 대응 목표 일수 — 유지보수 계약이 덮는 자산의 장애(수리) 접수 후 온사이트 대응·완료 시한(영업일).
   *  덮는 자산의 열린 수리가 이 일수를 넘기면 SLA 위반으로 판정한다(SLA 이행 독촉의 측정 기준). 미설정이면 위반 판정 안 함. */
...
/** 사용자가 스스로 올리는 신청 종류 — 재상신·상신 취소·결과 통보가 함께 쓰는 한 묶음이다.
...
export interface Approval {
  ...
  fulfilled?: boolean
  /** 승인했으나 집행할 수 없어 종결된 사유 — 요청~승인 사이에 대상이 사라지거나(대여·불출로 빠짐·폐기
   *  절차 진입·NAC 격리) 조건이 바뀐 경우. 승인 상태로 두고 fulfilled 를 세우지 않으면 결재 이력 반출이
   *  그 건을 영원히 '집행 대기'로 내보낸다 — 이미 신청자에게 통보하고 닫은 일을 미결 의무로 주장하는 셈이다.
   *  집행되지 않았으므로 fulfilled 로 덮을 수도 없다(그건 거짓이다). 제3의 상태로 남긴다. */
  unfulfilledReason?: string
  unfulfilledAt?: string
...
export interface IntakeLot {
  ...
  expectedDate?: string
  /** 검수 반려 사유(불량 내용) — 반려는 사유가 필수인데 그동안 감사로그와 공급사 통지 제목에만 남고
   *  로트에는 붙지 않았다. 반려 로트 앞에는 '재검수(교체품 도착)'와 '반품 완료(교체 없음)'가 나란히 놓이는데,
   *  둘 중 무엇을 고를지는 왜 반려됐는지에 달려 있다. 판단 근거가 그 화면에 있어야 한다. */
  rejectReason?: string
  /** 반품 완료(교체 없이 종결) 시점·처리자 — 종결 전이는 누가·언제를 남긴다(결재 decidedBy·폐기 wipedBy·
   *  계약 해지 terminatedBy 와 같은 규약). 반품 종결은 공급사 대금 환불·품질 평가의 기준 시점인데
   *  그동안 로트에는 날짜조차 없어 '언제 끝났는가'를 전역 감사로그에서만 알 수 있었다.
   *  inspector 는 반려한 사람이라 종결자와 다를 수 있어 대신 쓸 수 없다. */
  closedAt?: string
  closedBy?: string
...
export interface ScanRun {
   ...
   *  중단 사유를 여기 담지 않는다: 화면이 이 값을 '시간대 밖'으로 표기하므로, 시간대 안에서 중단한 회차까지
   *  안전장치를 우회한 것으로 읽힌다(감사관 앞에 거짓 지적을 놓는 셈이다). 중단 사유는 abortReason 이다. */
  override?: string
  /** 중단 사유 — 운영 민원·영향 우려로 회차를 도중에 멈춘 근거. 중단 회차는 대상 범위를 다 돌지 못했으므로
   *  그 자체가 '재실행 필요' 신호다(화면이 미완임을 밝히고 재실행으로 유도한다). */
  abortReason?: string
}
