...
/** 재배치 가능(가용) 자산 풀 — 상태 '유휴' AND 폐기 절차(완료 제외) 미진입. 재불출·재배치의 단일 정의.
 *  폐기 선정된 유휴 자산은 재불출 대상이 아니므로 제외한다. 안전재고 경보(lowStockCategories)와
...
export function availableAssets(assets: Asset[], disposals: DisposalRecord[]): Asset[] {
  ...
  return assets.filter((a) => a.status === '유휴' && !pendingDisposal.has(a.assetNo))
}

/** 배정(불출) 가능 재고 — 가용 유휴 재고 + 검수중(도입 직후 미배정분). 불출 가드(dispatchAsset)와 같은 판정이라
 *  불출 화면이 추천·집계한 자산을 서버가 '폐기 절차 중'으로 거부하는 막다른 길이 생기지 않는다. 서버 전용. */
export function assignableAssets(assets: Asset[], disposals: DisposalRecord[]): Asset[] {
  const pendingDisposal = pendingDisposalNos(disposals)
  return assets.filter((a) => ['유휴', '검수중'].includes(a.status) && !pendingDisposal.has(a.assetNo))
}
