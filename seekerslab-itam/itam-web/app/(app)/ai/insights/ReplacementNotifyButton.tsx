...
export function ReplacementNotifyButton({ n }: { n: number }) {
  const [msg, setMsg] = useState<string | null>(null)
  const [pending, startTransition] = useTransition()
  return (
    <span className="hstack" style={{ gap: 8, alignItems: 'center' }}>
      <button className="btn sm pri" disabled={pending || n === 0}
        onClick={() => startTransition(async () => setMsg((await notifyReplacement()).message))}
        title={n === 0 ? '오늘 통보할 교체 대상이 없습니다 (당일 발송분 제외)' : '교체 대상 자산의 소유 부서에 교체 검토를 요청 — 당일 중복 발송은 차단됩니다'}>교체 검토 통보 ({n})</button>
