...
function detectReportIntent(q: string): { kind?: ReportKind } | null {
  ...
  const kindMatchers: [RegExp, ReportKind][] = [
    ...
    [/복합\s*위험|복합위험|다중\s*이슈|중첩\s*(위험|신호)|위험\s*신호\s*중첩/i, '복합 위험 자산'],
    ...
    [/감가상각|상각\s*명세|상각\s*(스케줄|일정|계획)|depreciation/i, '감가상각 명세'],
...
function stubAnswer(question: string, userName: string, isUser: boolean, role: Role): ChatMessage {
  ...
  if (!isUser && (q.includes('결재') || q.includes('승인 대기') || q.includes('상신'))) {
    ...
    // 내가 지금 결재할 수 있는 건 — 대시보드 '내 결재 차례'와 같은 한 판정(canDecideApproval — 직무 분리 포함)을
  ...
  //  가용 판정은 lib/stock.availableAssets 단일 소스(유휴 AND 폐기 절차 미진입 AND 격리 아님)로 재고 경보와 어긋나지 않는다.
  if (canAsset && (q.includes('유휴') || q.includes('재배치') || q.includes('놀고 있는') || q.includes('놀리는') || q.includes('재불출') || q.includes('가용 자산') || q.includes('미배정 자산'))) {
    ...
    return {
      role: 'assistant',
      text: avail.length === 0
        ? '재배치 가능한 유휴 자산이 없습니다 (유휴 상태이며 폐기 절차 미진입·NAC 격리 아님 기준).'
        : `재배치 가능한 유휴 자산입니다 (유휴 · 폐기 절차 미진입 · 격리 아님 · ${avail.length}대 · 재불출/재배치 풀).\n\n${catRows
  ...
  // 담당자(스태프) QnA 답변 대기 — 사용자가 올린 문의 중 아직 답변 안 된 건(답변 작성 책무). 사용자 본인 QnA 인텐트(isUser)의 담당자 짝.
  //  대시보드 QnA 카드·게시판과 같은 미답변 판정(!p.answer). 운영 대기 큐(결재·리스크)와 같은 '내가 처리할 것' 축을 어시스턴트도 답한다.
  ...
  if (!isUser && (q.includes('문의') || q.includes('qna') || q.includes('질문') || q.includes('답변 대기') || q.includes('미답변'))) {
    ...
    return {
      ...
      text: pending.length === 0
        ? `답변 대기 중인 QnA 문의가 없습니다 — 등록된 문의 ${qna.length}건 모두 답변 완료됐습니다.`
        : [
            `답변 대기 QnA 문의 ${pending.length}건입니다 (전체 ${qna.length}건 중 · 등록 오래된 순).`,
            ``,
            ...pending.slice(0, 12).map((p) => `· ${p.title} — ${p.author}${p.category ? ` (${p.category})` : ''} · 등록 ${p.createdAt}`),
            pending.length > 12 ? `… 외 ${pending.length - 12}건` : ``,
          ].filter(Boolean).join('\n'),
      evidence: [{ label: '게시판 QnA (답변 작성)', href: '/board/qna' }],
    }
  }
