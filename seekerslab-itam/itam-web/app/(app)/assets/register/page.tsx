...
import { assetDataIssues, deptOfOwner, hasDataIssue } from '@/lib/quality'
...
import { availableAssets, pendingDisposalNos } from '@/lib/stock'
...
export default async function AssetRegisterPage({ searchParams }: { searchParams: Promise<{ q?: string; sel?: string; cat?: string; status?: string; warranty?: string; dq?: string; os?: string; crit?: string; maint?: string; spof?: string; replace?: string; stale?: string; receipt?: string; loanext?: string; loanret?: string; risk?: string; impact?: string; live?: string; avail?: string }> }) {
  const session = await requireView('/assets/register', 'USER', 'ASSET_MGR', 'SEC_MGR', 'ADMIN')
  const { q, sel, cat, status, warranty, dq, os, crit, maint, spof, replace, stale, receipt, loanext, loanret, risk, impact, live, avail } = await searchParams
  const s = getStore()
  ...
  const returnNeedsApproval = requiresApproval('반납')  // 회수(반납 처리) 직접 실행 게이트 — 서버 액션과 같은 판정
  ...
  const deptOf = deptOfOwner(s.users)
  const dqNos = scoped.filter((a) => hasDataIssue(a, deptOf)).map((a) => a.assetNo)
  // 상세 배너가 쓸 이슈 목록도 서버에서 만든다 — 보유자↔부서 불일치 판정에 사용자 대장이 필요한데,
  //  그걸 클라이언트로 내리면 사용자(USER)에게 조직 명부가 새고 역할마다 배너 내용이 갈린다.
  const dqIssues: Record<string, string[]> = {}
  for (const no of dqNos) {
    const a = scoped.find((x) => x.assetNo === no)
    if (a) dqIssues[no] = assetDataIssues(a, deptOf)
  }
  ...
  // 의존 영향 통지 버튼 건수 — 저하·이탈 상위 자산 중 아직 오늘 통지하지 않은 하위 담당 부서가 남은 건수(lib/reminders · 액션과 같은 소스).
  const impactNoticeCount = canManage ? impactNoticeTargets().length : 0
  // 통지 대상 상위 자산번호 — 대시보드 큐(?impact=1) 드릴다운이 큐 건수와 같은 집합을 열도록 같은 소스를 쓴다.
  const impactNos = canManage ? impactNoticeTargets().map((x) => x.asset.assetNo).filter((no) => scopedNoSet.has(no)) : []
  ...
  // 가용 재고(재배치 가능) 자산번호 — 재고 화면 안전재고 경보가 세는 그 집합과 같은 lib/stock 판정.
  //  경보의 드릴다운이 status=유휴 로만 열리면 폐기 선정·NAC 격리된 유휴 자산이 목록에 남아, 경보가 말한 수와
  //  목록 건수가 어긋난다(가용 0 이라며 링크를 누르면 한 대가 보인다).
  const availNos = availableAssets(scoped, s.disposals).map((a) => a.assetNo)
  ...
  return (
    <>
      ...
      {canManage && <BulkImport locations={activeLocations()} />}
      <Card pad={false}>
        <RegisterView assets={scoped} initialQuery={q ?? ''} canEdit={session.role !== 'USER'} canConfig={['ASSET_MGR', 'ADMIN'].includes(session.role)} canDoc={['ASSET_MGR', 'ADMIN'].includes(session.role)} canQuarantine={['SEC_MGR', 'ADMIN'].includes(session.role)} canManage={canManage} loanNeedsApproval={loanNeedsApproval} moveNeedsApproval={moveNeedsApproval} returnNeedsApproval={returnNeedsApproval} terminatedContracts={terminatedContracts} canExport={canExport('assets', session.role)} initialSel={initialSel} staleNos={staleNos} initialStale={stale === '1'} warrantyNos={warrantyNos} initialWarranty={warranty === 'soon'} expiryWindowDays={s.opsPolicy.expiryWindowDays} maintenanceWindowDays={s.opsPolicy.maintenanceWindowDays} dqNos={dqNos} dqIssues={dqIssues} initialDq={dq === '1'} eolNos={eolNos} initialEol={os === 'eol'} critNos={critNos} initialCrit={crit === '1'} contracts={s.contracts.filter((c) => c.status !== '해지').map((c) => ({ id: c.id, name: c.name, kind: c.kind }))} today={today()} initialCat={cat} initialStatus={status} receiptPendingCount={receiptPendingCount} receiptNos={receiptNos} initialReceipt={receipt === '1'} loanExtNos={loanExtNos} initialLoanExt={loanext === '1'} loanRetNos={loanRetNos} initialLoanRet={loanret === '1'} maintenanceNos={maintenanceNos} initialMaint={maint === '1'} maintOverdueCount={maintOverdueCount} eolNoticeCount={eolNoticeCount} impactNoticeCount={impactNoticeCount} impactNos={impactNos} initialImpact={impact === '1'} spofNos={spofNos} initialSpof={spof === '1'} replaceNos={replaceNos} initialReplace={replace === '1'} riskNos={riskNos} initialRisk={risk === '1'} initialLive={live === '1'} availNos={availNos} initialAvail={avail === '1'} disposalNos={disposalNos} licenseSeatsByAsset={licenseSeatsByAsset} users={session.role === 'USER' ? undefined : s.users.map((u) => ({ name: u.name, dept: u.dept }))} />
      </Card>
    </>
  )
}
