...
const TOGGLES: { field: 'scopeFilter' | 'autoApprove' | 'feedbackLearning'; label: string; desc: string; safeWhen: boolean }[] = [
  { field: 'scopeFilter', label: '권한 범위 필터', desc: '질의 컨텍스트에서 사용자 권한 밖 데이터를 원천 배제 — 코드가 항상 적용(회수 불가)', safeWhen: true },
        ...
        <div>
          ...
          <div className="vstack" style={{ gap: 8 }}>
            ...
            {TOGGLES.map((t) => {
              ...
              const locked = (LOCKED_AI_POLICY_TOGGLES as readonly string[]).includes(t.field)
              return (
                <div key={t.field} className="hstack" style={{ justifyContent: 'space-between', gap: 14, padding: '9px 12px', border: '1px solid var(--line)', borderRadius: 8, background: risky ? 'var(--err-bg)' : '#fff' }}>
                  ...
                  {locked ? (
                    <span className="chip ok" style={{ flex: 'none' }} title="회수 불가 — 질의 컨텍스트 권한 스코핑은 코드가 항상 적용합니다">ON 🔒</span>
                  ) : (
