...
async function guard() {
  ...
  if (!['ASSET_MGR', 'ADMIN'].includes(session.role) || !can('계약 · 라이선스', '저장', session.role)) {
    ...
    appendDenial({ actor: session.name, action: '권한 밖 변경 시도 — 계약 · 라이선스 (저장)', target: '/inventory/contracts' })
    return null
  }
  return session
}
