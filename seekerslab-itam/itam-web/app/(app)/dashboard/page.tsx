...
import { DEGRADED_CONNECTOR_STATUSES, USER_REQUEST_KINDS, isApprovalVisibleToUser, isOpenExposure, isReceiptPending, isUntriagedDiscovery } from '@/lib/types'
...
export default async function DashboardPage({ searchParams }: { searchParams: Promise<{ notices?: string; upcoming?: string }> }) {
  ...
  const myReceipts = s.assets.filter((a) => isReceiptPending(a) && a.owner === session.name)
  ...
  if (['ASSET_MGR', 'ADMIN'].includes(session.role)) {
    ...
    opsQueues.push(
      ...
      { label: '수령 미확인 (불출 후 인수 대기)', count: s.assets.filter(isReceiptPending).length, href: '/assets/register?receipt=1', tone: 'warn' },
      ...
      { label: '장기 미실측 (재물조사 편성 대기)', count: staleComposeTargets().length, href: '/inventory/survey-plan#stale', tone: 'warn' },
      // 재물조사 기한 경과 — 기한이 지난 미완료(계획·진행중) 조사 회차. 회차 표의 '기한 경과' 표시를 담당자 일과 시작점으로 끌어올려 독촉으로 닫는다(로59).
      { label: '재물조사 기한 경과 (담당자 독촉)', count: s.inventoryRounds.filter((r) => r.status !== '완료' && r.dueDate < today()).length, href: '/inventory/survey-plan#rounds', tone: 'err' },
