...
export default async function DashboardPage() {
  ...
  if (['ASSET_MGR', 'ADMIN'].includes(session.role)) {
    ...
    opsQueues.push(
      ...
      { label: '반납 신청 대기 (회수·점검 후 반환 접수)', count: s.assets.filter((a) => a.returnRequest).length, href: '/assets/register?status=대여중', tone: 'warn' },
      ...
      { label: '라이선스 미설치 좌석 (좌석 회수 후보 · 비용 절감)', count: buildLicenseUsage().totalUnusedSeat, href: '/inventory/contracts', tone: 'warn' },
