...
import { daysUntil, fmtAmount, NUM_LOCALE, SORT_LOCALE } from '@/lib/dates'
...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!['ASSET_MGR', 'ADMIN'].includes(session.role)) return forbidden(session.name, '권한 밖 문서 발급 시도 — 계약 카드', '/api/contract-card')
  // 매트릭스 '조회'도 만족해야 한다 — 화면(requireView)은 매트릭스를 보는데 문서 API 가 역할만 보면,
  //  조회 권한을 회수한 뒤에도 인쇄 문서로 같은 데이터가 그대로 나간다(화면은 막혔는데 API 는 열린 상태).
  if (!can('계약 · 라이선스', '조회', session.role)) return forbidden(session.name, '권한 밖 문서 발급 시도 — 계약 카드', '/api/contract-card')
  ...
  const maintHtml = c.kind === '유지보수' ? `
    ...
    ${(c.costs ?? []).length
      ? `<table><thead><tr><th>일자</th><th>항목</th><th class="r">금액</th></tr></thead><tbody>${[...c.costs!].sort((x, y) => y.date.localeCompare(x.date, SORT_LOCALE)).map((ct) => `<tr><td class="d">${esc(ct.date)}</td><td>${esc(ct.item)}</td><td class="r">${ct.amount.toLocaleString(NUM_LOCALE)}원</td></tr>`).join('')}</tbody></table>`
