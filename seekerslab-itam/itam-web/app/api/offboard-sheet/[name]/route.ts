...
export async function GET(_req: Request, { params }: { params: Promise<{ name: string }> }) {
  ...
  if (!['ASSET_MGR', 'ADMIN'].includes(session.role)) return forbidden(session.name, '권한 밖 문서 발급 시도 — 오프보딩 명세서', '/api/offboard-sheet')
  // 매트릭스 '조회'도 만족해야 한다 — 화면(requireView)은 매트릭스를 보는데 문서 API 가 역할만 보면,
  //  조회 권한을 회수한 뒤에도 인쇄 문서로 같은 데이터가 그대로 나간다(화면은 막혔는데 API 는 열린 상태).
  if (!can('자산 대장', '조회', session.role)) return forbidden(session.name, '권한 밖 문서 발급 시도 — 오프보딩 명세서', '/api/offboard-sheet')
