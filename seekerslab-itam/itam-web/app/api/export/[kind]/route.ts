...
/** 쉼표 목록 파라미터 — 없으면 undefined(좁히지 않음), 있으면 배열(빈 배열이면 0건으로 좁힘).
 *  화면이 0건으로 좁혀진 채 반출하면 'ids=' 처럼 빈 값이 실려 온다. 이걸 파라미터가 아예 없는 것과
 *  같게 읽으면 '아무것도 좁히지 않음'이 되어 대장 전량이 나간다 — 버튼에는 (0)이라 적혀 있는데
 *  파일에는 전 건이 담기는, 화면과 가장 크게 어긋나는 반출이 된다. 두 경우를 구분해 빌더까지 넘긴다. */
function csvParam(v: string | null): string[] | undefined {
  return v === null ? undefined : v.split(',').map((x) => x.trim()).filter(Boolean)
}
...
export async function GET(req: Request, { params }: { params: Promise<{ kind: string }> }) {
  ...
  const ids = csvParam(sp.get('ids'))
  const idScope = (sp.get('scope') ?? '').trim().slice(0, 200)
  const selExport = k === 'assets' && nos !== undefined
  ...
  const idFiltered = ids !== undefined
  ...
  const scopeText = selExport ? `선택 ${nos?.length ?? 0}건`
    ...
    : idFiltered ? `필터: ${idScope || `화면 필터 — ${ids?.length ?? 0}행`} (${ids?.length ?? 0}행)`
    : ''
