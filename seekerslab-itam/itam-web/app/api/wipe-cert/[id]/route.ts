...
export async function GET(
  ...
  if (!['ASSET_MGR', 'ADMIN'].includes(session.role)) {
    return forbidden(session.name, '권한 밖 문서 발급 시도 — 폐기 증적', '/api/wipe-cert')
  }
  // 매트릭스 '조회'도 만족해야 한다 — 화면(requireView)은 매트릭스를 보는데 문서 API 가 역할만 보면,
  //  조회 권한을 회수한 뒤에도 인쇄 문서로 같은 데이터가 그대로 나간다(화면은 막혔는데 API 는 열린 상태).
  if (!can('수명주기', '조회', session.role)) return forbidden(session.name, '권한 밖 문서 발급 시도 — 폐기 증적', '/api/wipe-cert')
  ...
  const body = [
    ...
    `- 소거 일시: ${d.wipedAt}`,
    `- 처리자: ${d.wipedBy}`,
    d.disposition ? `- 물리 처분: **${d.disposition}**${d.proceeds ? ` (매각 대금 ${d.proceeds.toLocaleString(NUM_LOCALE)}원)` : ''}` : '',
    '',
    '## 확인',
