...
try {
  ...
  //  회차 등록 폼도 같은 규칙을 말해야 한다 — 폼은 '폐기 완료 제외'만 적어 두어, 담당자가 전사로 회차를 만들면
  //  기대한 모수(대장 자산 − 폐기완료)보다 적은 수가 잡힌다(SW·가상자원이 함께 빠진다). 수를 설명 없이 줄이지 않는다.
  check('재물조사 계획: 회차 등록 폼이 편성 모수 규칙을 밝힌다',
    planHtml.includes('현장에서 스캔할 실물이 있는 유형(단말·서버·네트워크·주변기기)만'), '등록 폼 모수 안내가 실제 편성 규칙과 다름')
