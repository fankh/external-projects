...
try {
  ...
  // 대여 반환 기한 — 수리 의뢰 열은 있는데 대여는 없어, 반출본만으로는 대여중 자산이 언제 돌아오는지·연체인지 알 수 없었다.
  //  기한을 리터럴로 박으면 시드가 상대 날짜로 바뀌거나 그 날이 지날 때 검사만 깨진다 — 인쇄 카드가 말한 기한을
  //  반출본도 그대로 싣는지 대조한다(두 산출물이 같은 기한을 말하는가).
  const loanCardDue = (/반환 기한[^0-9]*(\d{4}-\d{2}-\d{2})/.exec(cardLoanOk) || [])[1] ?? ''
  check('자산 대장 엑셀: 대여 반환 기한 컬럼 반출(정상 기한 · 카드와 동일)',
    asBuf.includes('대여 반환 기한') && loanCardDue.length === 10 && asBuf.includes(loanCardDue),
    `카드 기한=${loanCardDue || '추출 실패'}`)
  ...
  // 편성 대상은 현장에서 스캔할 실물이 있는 유형만 — SW·가상자원을 회차에 넣으면 영원히 미스캔으로 남아
  //  진행률 분모만 부풀리고, 마감 시 미스캔분이 분실 후보로 올라와 클라우드 VM 을 '분실'로 지목하게 된다.
  ...
  // ── 정책 강도가 실행을 제약하는가 — 화면이 "강도 정책 안에서만 실행되며"라고 약속한다 ──
  //  그동안 정책의 강도는 표에 표시만 되고 실행을 제약하지 않았다: 운영자가 콘솔에서 아무 강도나
  //  골라 그대로 돌았고, 관리자가 운영망 부하를 줄이려 채널 강도를 낮춰도 아무 일도 일어나지 않았다
  //  (정책 강도 보통인 네트워크 능동 스캔에서 높음 실행이 완료되는 것을 실제로 확인했다).
  //  시간대 가드와 같은 범위(능동 채널)에만 적용한다 — 패시브·로그 수집에는 부하 개념이 없다.
  const scanActSrc = readFileSync(path.join(ROOT, 'app', '(app)', 'discovery', 'scan', 'actions.ts'), 'utf8')
  const scanConSrc = readFileSync(path.join(ROOT, 'app', '(app)', 'discovery', 'scan', 'ScanConsole.tsx'), 'utf8')
  check('스캔 강도: 정책 강도를 상한으로 서버가 강제', /INTENSITY_ORDER\.indexOf\(input\.intensity\) > INTENSITY_ORDER\.indexOf\(p\.intensity\)/.test(scanActSrc))
  check('스캔 강도: 상한 판정은 능동 채널만 (시간대 가드와 같은 범위)', scanActSrc.includes("p.kind === '능동' && INTENSITY_ORDER"))
  ...
  const obSheet2 = await (await get(`/api/offboard-sheet/${encodeURIComponent(String.fromCharCode(54620,46020,50980))}`, 'ADMIN')).text()
  check('오프보딩 명세서: 반납대기 보유(회수 진행 중)가 실린다',
  //  자산번호만 보면 미처리 상신 결재 섹션의 반납 상신 제목에도 그 번호가 있어 검사가 헛돈다(실제로 그랬다) —
  //  새 섹션에서만 나오는 상태 문자열까지 함께 본다.
    obSheet2.includes('이름이 남아 있는 자산') && obSheet2.includes('반납대기'), '반납대기 보유가 명세서에서 빠짐')
