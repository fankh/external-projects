/* 샘플 산출물 재생성 — docs/샘플_*.{md,csv} 전 종류를 실행 중인 플랫폼이 만든 결과물로 다시 저장한다.
...
const REPORTS = [
  ...
  ['계약 관리 현황', '샘플_계약관리현황'],
  // 아래 일곱은 그동안 샘플이 없어 드리프트 검사가 닿지 않았다 — 제품이 내주는 리포트인데 생성 결과를
  //  아무도 고정하지 않은 상태였다(스모크의 '리포트 샘플 10종 일치'는 샘플 수만 셌지, 종류 수와 맞대지 않았다).
  ['정보보호 컴플라이언스 증적', '샘플_정보보호컴플라이언스증적'],
  ['라이선스 갱신·트루업 계획', '샘플_라이선스갱신트루업계획'],
  ['단일 장애점·영향 분석', '샘플_단일장애점영향분석'],
  ['자산 운영 리스크', '샘플_자산운영리스크'],
  ['복합 위험 자산', '샘플_복합위험자산'],
  ['감가상각 명세', '샘플_감가상각명세'],
  ['계약 갱신 전망', '샘플_계약갱신전망'],
]
...
await browser.close()
server.kill()
// 종류 수는 REPORTS 에서 센다 — 손으로 적어 두면 종류를 늘렸을 때 조용히 어긋난다(다른 수치 주장과 같은 규약)
if (CHECK) console.log(failed ? `\n드리프트 ${failed}건 — 'node scripts/gen-samples.mjs' 로 재생성 후 커밋하세요` : `\n${REPORTS.length}종 샘플 최신 — 드리프트 없음`)
else console.log(failed ? `\n실패 ${failed}건 — 커밋 전 확인 필요` : `\n${REPORTS.length}종 재생성 완료 — git diff 로 드리프트 확인`)
