...
class GuardrailPipeline {
   ...
   * @param {string} [params.provider] LLM provider override
   ...
   */
  static async process({ content, tenantId, userEmail, allowCategories, provider, model, ipAddress }) {
    const timings = {};
    ...
    // 3) LLM call through the white-listed gateway
    let llm;
    try {
      llm = await LlmGateway.chat({ messages: [{ role: 'user', content: sentPrompt }], provider, model });
      timings.llmMs = lap();
    } catch (err) {
      timings.llmMs = lap();
      await GuardrailAuditTrail.record({
        tenantId, userEmail, verdict: 'blocked', stage: 'gateway', prompt: content,
        detail: { error: err.code || err.message, timings, ipAddress }
      });
      throw err;
    }
