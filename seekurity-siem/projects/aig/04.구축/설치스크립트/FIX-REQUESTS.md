...
| 번호 | 제목 | 상태 | 조치 주체 | 영향 |
|---|---|---|---|---|
| FIX-001 | 콘솔 Parser 탭 클릭 시 화면 오류 | 적용됨 | 수행사 | 파서 확인 불가 |
| FIX-002 | SQL 직접 등록 시 NULL 로 대시보드 500 | 적용됨 | 수행사 | 대시보드 전체 실패 |
| FIX-003 | Syslog Receiver 로그 파일 권한 | **요청** | 수행사 | 장애 추적 불가 |
| FIX-004 | 파서 필드명이 제품 표준과 불일치 | 적용됨 | 수행사 | 로그 검색 화면 공백 |
| FIX-005 | 파서 추출 범위 부족 (ICMP·protocol 등) | 적용됨 | 수행사 | 일 529건 누락 |
| FIX-006 | 기본 룰 56개 중 2개만 동작 가능 | **요청** | 수행사 | 탐지 미작동 |
| FIX-007 | 탐지 엔진이 룰 조건을 적용하지 않음 | **요청** | 수행사·벤더 | **오탐 대량 발생 위험** |
| FIX-008 | 룰 삭제 시 탐지 이력이 고아로 남음 | 적용됨 | 수행사 | 화면 건수 불일치 |

### 우선순위

1. **FIX-007** — 조건이 평가되지 않으면 룰은 임계치만으로 발화합니다.
   현재 기본 룰을 활성 상태로 두면 정상 트래픽에도 탐지가 쏟아집니다.
   **FIX-006 보다 먼저 확정해야 하며**, 결과에 따라 FIX-006 의 조치 내용 자체가 달라집니다.
2. **FIX-006** — FIX-007 확정 후 진행합니다.
3. **FIX-003** — 운영 영향은 작으나 장애 추적성 확보를 위해 필요합니다.

### 반복된 원인 — 콘솔 API 우회

FIX-001, FIX-002 는 같은 뿌리에서 나왔습니다. **콘솔 API 를 거치지 않고 DB 에 직접
등록하면 서버가 채워 주던 기본값이 비어 UI·API 가 깨집니다.** FIX-007 도 같은 계열일
가능성이 있습니다(조건이 API 경로로 등록되어야 반영될 수 있음).

가능하면 `/log/source`, `/log/parser` 등 API 경로를 사용하고, DB 직접 조작은
관리자 비밀번호를 모르는 등 불가피한 경우로 한정하는 것이 좋습니다.

### 제품 측 전달 대상

| 번호 | 내용 |
|---|---|
| FIX-001 | 프런트엔드가 선택 필드 `timeFormat` 에 방어 없이 `.trim()` 호출 |
| FIX-002 | nullable 컬럼을 primitive 로 매핑해 한 행의 NULL 로 목록 조회 전체가 500 |
| FIX-006 | 기본 룰 마이그레이션 SQL 이 배포 스키마에 없는 `ON CONFLICT (uuid)` 사용 |
| FIX-006 | 기본 룰이 참조하는 `deviceAction` 필드를 제품이 생성하지 않음 |
| FIX-007 | 탐지 엔진이 `rule_conditions` 를 적용하지 않는 것으로 관측됨 |
| FIX-008 | 탐지 화면의 건수와 목록이 다른 기준으로 조회됨 |
