...
export default async function ExpensePage() {
  ...
  const canConfirm = canManage || groupGrantsAction(me.name, '/finance/expense', 'confirm')
    ...
    <>
      ...
      <Card title="경영계획 — 비용 항목" pad={false}>
        <div className="tbl-wrap">
          <table className="tbl">
            <thead><tr><th>항목번호</th><th>항목명</th><th>담당</th><th className="num">계획액 (만원)</th><th>상태</th>{(canManage || canConfirm) && <th className="c">확정</th>}</tr></thead>
            <tbody>
              {plans.map((p) => (
                <tr key={p.id}>
                ...
                return (
                  <tr key={p.id}>
                    ...
                    {/* 초과(err)는 반올림 집행률이 아니라 집행액>계획액 원값으로 판정 — 반올림 집행률(rate)로
                        ...
                        투자 화면·KPI 초과 톤이 쓰는 원값 비교와 일치. >=90 접근 경고는 표시용 소프트 임계라 반올림 유지. */}
