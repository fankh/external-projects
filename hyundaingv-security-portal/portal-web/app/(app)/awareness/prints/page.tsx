...
async function importDaily() {
  ...
  try {
    ...
    for (const row of rows) {
      // 실 고객사 secdata 어댑터가 계약을 어겨 오형 행을 반환해도 스토어에 넣지 않는다 — 자동 배치(notify.ts,
      // v1.5.71)와 동일 수집 계약 검증. name·document·printedAt(식별·수신자) 외에 dept·pages 도 검증한다 —
      // {p.dept}·{p.pages} 를 화면이 직접 렌더하므로 객체값이면 React child 500(인메모리 주입은 loadFromFile
      // 정규화를 우회한다). 형태 어긋난 행은 건너뛴다.
...
async function submitDiscards() {
  ...
  draftApproval({ docType: '출력물폐기 상신', title: `[출력물폐기] ${me.name} ${targets.length}건 (${today()})`, ref, drafter: me, items: targets.map((p) => p.id) })
