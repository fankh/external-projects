...
export interface Approval {
  ...
  approvedBy?: string[]
  /** 묶음 상신 항목 스냅샷 — 장애보고·출력물폐기·서약현황 묶음은 상세를 역링크(reportRef·approvalRef)로
   *  재구성하는데, 반려 시 역링크가 초기화돼 상세가 'N건'→0건으로 빈다(§VI 감사 완결성). 상신 시점의 항목
   *  id 를 저장해 역링크 없이도 재구성한다(id 조회). 비묶음 문서는 생략. */
  bundledIds?: string[]
